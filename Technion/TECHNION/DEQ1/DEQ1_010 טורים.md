...
1. אם $q=\lim_{ n \to \infty }\sqrt[n]{ |a_{n}| }<1$. תהי $\tilde q= \frac{1+q}{2}$ ו- $\varepsilon= \frac{1-q}{2}$. מתקיים:
	...
	ולכן $0\leq |a_{n}|\leq (\tilde q)^{n}$. כיוון ש- $0\leq\tilde q <1$, אז $\sum_{n=1}^{\infty}(\tilde q)^{n}$ מתכנס (ראו דוגמא ראשונה לאחר הגדרת [[#טור|הטור]], ולכן ממבחן ההשוואה נובע כי $\sum_{n=1}^{\infty}|a_{n}|$ מתכנס ולכן $\sum_{n=1}^{\infty}|a_{n}|$ מתכנס $\impliedby$ $a_{n}\xrightarrow[n\to \infty]{}0$.
