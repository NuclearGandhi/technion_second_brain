...
### השפעת העלאה בחזקה והפעלת פולינום על מטריצה - על הע"ע שלה

>[!info] טענה:
>אם $A\cdot \underbrace{ v }_{ \neq 0 }=\lambda \cdot v$, אז:
>1. $$\forall n\in \mathbb{N}: A^{n}\cdot v=\lambda^{n}\cdot v$$
>2. אם $B=p(A)$, אז הע"ע של $B$ הם $\lambda_{B}=p(\lambda _{A})$, עם אותם ו"ע $v$.
