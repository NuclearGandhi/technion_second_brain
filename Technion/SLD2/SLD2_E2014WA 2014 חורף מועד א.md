...
### סעיף 20
נרצה למצוא את ${u}_{1}$ לפי שיטות אנרגיה. האנרגיה האלסטית האגורה בקורה נתונה ע"י:
$$\begin{aligned}
U & =\dfrac{1}{2}\int_{L} \dfrac{{{M}_{2}}^{2}}{E{I}_{33}} +\dfrac{{{M}_{3}}^{2}}{E{I}_{22}}\,\mathrm{d}{x}_{1} \\
\end{aligned}$$
...
$$\begin{gathered}
{S}_{11}=\dfrac{R^{3}}{EI}\left( \dfrac{3\pi}{4}-2 \right)
\end{gathered}$$
