...
נשים לב כי הנוסחה ל-$Q$ היא בעצם המונה של מיקום [[SLD1_006 מרכז כובד ומרכז מסה#מציאת מרכז כובד עבור גוף יחיד|מרכז הכובד]] של החתך $\tilde{A}$, ביחס לראשית הצירים שמרכזה במרכז הכובד של $A$:
$$
\begin{aligned}
...
בנקודה ${p}_{2}$, ה-$\tilde{A}$ הוא השטח המתואר באיור. ניתן לראות (בערך) שהמרכז כובד של הגאומטרייה נמצא בצד השלילי של $\underline{e_3}$. נסיק כי בנקודה זו $\tau$ גדל מאשר בקצה, וגודלו חיובי:
$$
