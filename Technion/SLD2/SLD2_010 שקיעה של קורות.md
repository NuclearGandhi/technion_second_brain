...
- להבי טורבינות הסובבים בתוך מיכל צילינדרי מתארכים ועשויים לפגוע במיכל אם לא נודא שהדפורמציות יהיו בגבולות המותר.
- גם אם רצפת אולם היתה מחזיקה מעמד מבחינת המאמצים המתפתחים בה, לא נוכל להרשות ששקיעתה תהיה גדולה. מצב כזה לא רצוי הן מהבחינה המעשית (העמדת רהיטים, שטיפת רצפות) והן מהבחינה הפסיכולוגית.

>[!notes] הערה: 
 >במסגרת הקורס נדון בעומסים הגורמים למאמצים בתחום האלסטי לינארי. השקיעה המקסימלית לא תעלה בד"כ על אחוז או שניים מאורך הקורה. השינויים הגיאומטריים החלים בקורה העמוסה יהיו זניחים וכל החישובים (כגון מהלך המומנטים) יערכו ביחס לגיאומטרית הקורה לפני העמסתה. השקיעה שתחושב היא שקיעת **המשטח הניטראלי** - המשטח החופשי ממאמצי מתיחה/לחיצה. הדפורמציות בעובי הקורה זניחות.
...
נביט בחתך באורך $\Delta {x}_{1}$ בקורה הנמצאת תחת כפיפה משופעת:
...
>1. הציר המקווקו העבור בקורה הוא הציר הניטראלי (כאשר אנו מניחים כי אין כוח צירי, ואז הציר הניטראלי עובר דרך ראשית הצירים).
...
בנוסף, נסמן את אורך הקשת של סיב חומרי שכיוונו $\underline{e_1}$ במיקום $\underline{e_2}$ ב-$S({x}_{2})$. אזי:
...
ניתן להראות (יעני אין לנו כוח להוכיח) שהעקמומיות של גרף מסוים $y$ מקיים:
$$\kappa=\dfrac{y''}{(1+(y')^{2})^{3/2}}$$

במקרה שלנו, הגרף הוא הזזות בכיוון $\underline{e_2}$, כלומר, ${u}_{2}$. לתזוזות אילו שמתרחשות עקב מומנטים, נקרא **שקיעה**. לכן:
$$\kappa=\dfrac{u_{2,11}}{(1+(u_{2,1})^{2})^{3/2}}\approx u_{2,11} \tag{2}$$
כאשר ה-$\approx$ הוא נכון כאשר אנו דנים בקורות שחוות דיפרומציות קטנות.
נציב את $(2)$ ב-$(1)$:
...
נבחר בקצה השמאלי:
$$u_{3}(0)=0,\, \quad EIu_{3,11}(0)=0$$
נבחר בקצה הימני:
$$u_{3}(L)=0,\, \quad EIu_{3,11}(L)=0$$
ונקבל כי:
>$$\boxed {
u_{3}=\dfrac{{q}_{0}}{EI}\left( \dfrac{1}{12}({x}_{1})^{4}-\dfrac{1}{6}({x}_{1})^{3}L+12{x}_{1}L^{3} \right)
 }$$
...
הפעם, הבעיה לא [[../SLD1/SLD1_002 שיווי משקל#מסויימות סטטית|מסוימת סטטית]]. חוץ מהעובדה שבריתום לקורה כוח התגובה בכיוון $\underline{e_1}$ מתאפס, לא נוכל לחשב בצורה ישירה התגובות האחרות כמו בדוגמה הקודמת. ניאלץ להשתמש בנוסחה:
$${q}_{3}=EI_{33}u_{3,1111}$$
...
{u}_{3}=\dfrac{{q}_{0}}{EI}\left( \dfrac{1}{24}({x}_{1})^{4}-\dfrac{5}{48}L({x}_{1})^{3}+\dfrac{1}{16}L^{2}({x}_{1})^{2} \right)
 ...
 עם הפתרון הזה, אנו למעשה נוכל לחשב כעת את התגובות בסמכים, שלא יכלנו לפני זה מאחר והבעיה לא מסוימת סטטית.
 נשרטט דג"ח חיצוני:
...
$${V}_{3}=-EI_{33}u_{3,111}$$
אבל אנחנו כבר יודעים את ${u}_{3}$. נוכל פשוט לגזור ולהציב כדי למצוא כי:
...
>[!notes] הערה: 
 ...
 >$$u_{3}\equiv w,\, \quad {u}_{2}\equiv v$$
