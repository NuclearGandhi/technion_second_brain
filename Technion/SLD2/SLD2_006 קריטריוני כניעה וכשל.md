...
2. לדיפורמציה פלסטית. כשל זה מתאפיין בהחלקה של מישורי החומר אחד כלפי השני, ולכן הוא לרוב יקרה כתוצאה ממאמץ גזירה.
...
נסקור בקצרה שלוש תיאוריות כשל: מאמץ נורמלי מקסימאלי (ראנקין); מאמץ גזירה מקסימאלי (טרסקה); ואנרגית שינוי צורה מכסימאלית (פון מיזס). מבין השלוש, רק האחרונה מניחה כי הכשל נקבע על ידי אינטראקציה בין רכיבי טנזור המאמצים בנקודה.
...
או עבור קריטריון ראנקין:
$$\sigma_{\text{eq}}=\sigma_{y}$$

## מקדם ביטחון
לפעמים נרצה להחמיר עוד יותר את הקריטריונים שלנו מלאכותית. נוח להשתמש **במקדם ביטחון $(K)$**, כאשר $K>1$. אז המאמץ האקוויולנטי:
