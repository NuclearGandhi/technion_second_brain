...
1. לשבר החומר ולהפרדתו למספר חלקים.
2. לדיפורמציה פלסטית.
...
נסקור בקצרה שלוש תיאוריות כשל: מאמץ נורמאלי מקסימאלי (ראנקין); מאמץ גזירה מקסימאלי (טרסקה); ואנרגית שינוי צורה מכסימאלית (פון מיזס). מבין השלוש, רק האחרונה מניחה כי הכשל נקבע על ידי אינטראקציה בין רכיבי טנזור המאמצים בנקודה.

# קריטריון ראנקין
**קריטריון ראנקין (Renkin)** הוא קריטריון לכשל בחומרים פריכים כמו ברזל יציקה. ע"פ תיאוריה זו, חומר נכשל כאשר מאמץ נורמלי, על חתך כלשהו בנקודה הנדונה, מגיע לערך הקריטי $\sigma_{y}$.
...
הקריטריונים שנעסוק בהם הינם מאוד גסים - לא מדויקים. אנחנו לוקחים משפחה מאוד גדולה חומרים פריכים ונותנים להם קריטריונים כלליים. באותה מידה עבור חומרים משיכים. קריטריון ראנקין הוא לא קריטריון גס, הוא קריטריון *גס מאוד*. לאט לאט ניתן קריטריונים טיפה פחות גסים, אבל נשים לב שככל שנרצה קריטריונים יותר מדויקים, נצטרך לבצע יותר ניסויים והרבה יותר חישובים.
...
**קריטריון טרסקה** מתבסס על ההבחנה כי דיפורמציה פלסטית מתרחשת בהשפעת מאמץ גזירה. לפיכך,יש לחשב מהו מאמץ הגזירה הגדול ביותר הקיים בנקודה בחומר ולהשוותו **למאמץ גזירה קריטי ($\tau_{y}$)** הגורם כניעה פלסטית בניסויי מעבדה.
...
כל עוד מצב המאמצים בחומר מתואר ע"י נקודה בתוך המצולע, אין כשל. כאשר מגיעים להיקף המצולע, מתרחש כשל פלסטי. נשים לב כי ברביע הראשון והשלישי, כאשר שני המאמצים הראשיים שווים זה לזה, קריטריון טרסקה חוזה כשל כמו קריטריון ראנקין. לעומת זאת, ברביעים השני והרביעי, היכן שסימני המאמצים הראשיים הפוכים זה לזה, קריטריון טרסקה חוזה כשל במאמצים נמוכים בהרבה מאמצי הכשל החזויים לפי קריטריון ראנקין.


>[!notes] הערה: 
 >קיבלנו כאן שקריטריון טרסקה הוא *יותר* מחמיר מקריטריון ראנקין, אבל זה *רק* במקרה של $\sigma^{(3)}=0$!
...
עבור קובייה איניפיטסימלית בעלת שטח חתך $A$ ואורך השינוי ${L}_{0}$:
...
נציב ב-$(1)$:
$$\boxed{\bigg[(\sigma^{(1)}-\sigma^{(2)})^{2}+(\sigma^{(1)}-\sigma^{(3)})^{2} +(\sigma^{(2)}-\sigma^{(3)})^{2} \bigg]=2\sigma_{y}^{2}}$$
...
לפי שיקולי כוחות:
$$\begin{gather}
0=\sum \underline{F}\cdot \underline{e_{\theta}}=\sigma_{\theta\theta}\cdot 2Lt-LR\int_{0}^{\pi} P \sin \varphi \, \mathrm{d}\varphi=\sigma_{\theta\theta}\cdot 2Lt-2LPR  \\
...
נסיק כי $\sigma_{rr}$ משתנה לאורך עובי הדפנה, כך ש- $-P<\sigma_{rr}<0$. מאחר ואנו מניחים שהעובי מאוד דק, נזניח את $\sigma_{rr}$:
$$\sigma_{rr}\approx 0$$
