...
void pid(void)
{     Enc1[0] = Enc[0]; Enc1[1] = Enc[1]; Enc1[2] = Enc[2]; Enc1[3] = Enc[3]; //store previews
	 Enc[0]=TIM1->CNT; Enc[1]=TIM8->CNT; Enc[2]=TIM3->CNT; Enc[3]=TIM4->CNT;
	 We[0] = Enc[0]-Enc1[0]; We[1] = Enc[1]-Enc1[1]; We[2] = Enc[2]-Enc1[2]; We[3] = Enc[3]-Enc1[3];
     set_pwm( Pwm   );
}
...
{
if (mode==DO_RECORD)
 {	REC[ka][2]= Pwm[ch]; // increment index
    REC[ka++][1]= Enc[ch]; // increment index

   if (ka==NP) {mode=DO_IDLE; //done recording
                ka=0; Pwm[0]=Pwm[1]=Pwm[2]=Pwm[3]=0;  pid(); //reset all
   }
 }
}
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim){
 if(htim->Instance == TIM15){
	 Pwm[ch]=0;
	   if (mode==DO_RECORD) {
		   if ((ka % 100)==0) s=-s;
		   if (ka>10) {Pwm[ch] = s*STEP_SIZE;}  else {Pwm[ch]=0;}
		   pid(); // update Pwm and read encoders
		   record_enc();
	    } //if mode==
	   if (--tic==0){  tic = 50; HAL_GPIO_TogglePin(GPIOA, GPIO_PIN_5); }
  } //if htim->
