function ui()
    ...
    % Ensure you have the correct COM port selected below.
    
    current_state = 1;

    % --- Configuration ---
    COM_PORT = 'COM10'; % CHANGE THIS to your actual COM port
    BAUD_RATE = 115200;
    
    % --- Serial Connection ---
    % Try to close existing connections to this port to avoid conflicts
    try
        old_objs = serielportfind('Port', COM_PORT);
        if ~isempty(old_objs)
            fclose(old_objs);
            delete(old_objs);
        end
    catch
        % Ignore errors if instrfind fails or no instrument control toolbox
    end
