%example_lesson_9_sCurve

% an example for Lab assignmen #2 , part 2
% 
%% Izhak Bucher 26-12-2025
%
% This example generate points on an S-curve.
...
[s,xs,ys,dxds,dyds,kappa,vt,tt,pp] = ...
    ...
    x,y,vt_min,vt_max,SamplesPerSegment= 2,VtEpsKappa=25e-2,...
    VtRefStrategy='max',Alpha=01 );


 vt(tt>=max(tt)/2) = -vt(tt>=max(tt)/2) ; 
clf
subplot(2,1,1)
plot(xs,ys )
axis tight
...
%% export to file
v_out = [vt;9999;9999];  % add zero speed and stopping symbol (9999)
t_out = [tt; tt(end)+.05; tt(end)+.1  ]
trajectory_export(v_out, t_out, 9, 'trajectory.c')
