...
### דרך ב'
נבחר במערכת הצירים צמודת המכונית $\und{e}_{i}'$.
נייצג את מיקום המכונית במערכת הצירים:
$$\und{r}_{C} =R \und{e}_{1}'Re$$
נגזור את המיקום לפי כלל האופרטור:
$$\und{v}_{C} =\frac{\delta}{\delta t} \und{r}_{C} +\und{\Omega}' \times \und{r}_{C}  $$
$$\frac{\delta}{\delta t} \und{r}_{C} =0$$

נמיר את $\omega$ למערת צירים $\und{e}_{i}'$:

$$\begin{cases}
\und{e}_{1}''=\cos (\lambda)\und{e}_{1}'-\sin (\lambda)\und{e}_{2}' \\[1ex]
\und{e}_{2}''=\sin (\lambda)\und{e}_{1}'+\cos (\lambda)\und{e}_{2}'
\end{cases}$$

$$\omega=p\und{e}_{2}''+\dot{\lambda}\und{e}_{3}'=p \sin (\lambda)\und{e}_{1}'+p \cos (\lambda)\und{e}_{2}'+\dot{\lambda}\und{e}_{3}'$$

$$\begin{aligned}
\und{\omega} \times \und{r}_{C}& =\begin{vmatrix}
 \und{e}_{1}' & \und{e}_{2}' & \und{e}_{3}' \\ 
 p \sin (\lambda) & p \cos (\lambda) & \dot{\lambda} \\ 
 R & 0 & 0 
\end{vmatrix} = \\
&=\dot{\lambda}R\und{e}_{2}'-Rp\cos (\lambda)\und{e}_{3}'
\end{aligned}$$

$$\und{v}_{C} =\dot{\lambda}R\und{e}_{2}'-Rp\cos (\lambda)\und{e}_{3}'$$

נגזור את הביטוי שוב בעזרת כלל האופרטור כדי למצוא את התאוצה:
$$\und{a}_{C} =\frac{\delta}{\delta t} \und{v}_{C} +\und{\omega}  \times \und{v}_{C} $$
$$\frac{\delta}{\delta t} \und{v}_{C} =\ddot{\lambda}R\und{e}_{2}'+\dot{\lambda} Rp \sin (\lambda)\und{e}_{3}'-\cancelto{0}{R\dot{p}\cos (\lambda)\und{e}_{3}'} $$
הרכיב $\ddot{p}$ מתאפס מכיוון ש - $p$ קבוע.
$$\begin{aligned}
\und{\omega}  \times \und{v}_{C} &=\begin{vmatrix}
 \und{e}_{1}' & \und{e}_{2}' & \und{e}_{3}' \\ 
 p \sin (\lambda) & p\cos (\lambda) & \dot{\lambda} \\ 
 0 & \dot{\lambda}R & -Rp\cos (\lambda) 
\end{vmatrix} = \\
&=\Big[-Rp^{2}\cos ^{2}(\lambda)-\dot{\lambda}^{2}R\Big]\und{e}_{1}'+\Big[Rp^{2}\sin (\lambda)\cos (\lambda)\Big]  \und{e}_{2}'+\Big[pR\dot{\lambda}\sin (\lambda)\Big]\und{e}_{3}' 
\end{aligned}$$

$$\und{a}_{C} =\Big[-Rp^{2}\cos ^{2}(\lambda)-\dot{\lambda}^{2}R\Big] \und{e}_{1}'+\Big[Rp^{2}\sin (\lambda)\cos (\lambda)+\ddot{\lambda}R\Big] \und{e}_{2}'+\Big[2\dot{\lambda}Rp \sin(\lambda) \Big]\und{e}_{3}' $$

בנוסף אנחנו יודעים שהמהירות היחסית בין המכונית וכדור הארץ היא $v$, לכן:
$$v=\dot{\lambda}R$$
מכיוון ש-$v$ היא מהירות קבועה, נקבל שגם $\dot{\lambda}$ מהירות קבועה, ולכן $\ddot{\lambda}=0$

ומכאן שהתאוצה היא:
$$\und{a}_{C} =\left[ -Rp^{2}\cos ^{2}(\lambda)-\frac{v^{2}}{R} \right] \und{e}_{1}'+\Big[Rp^{2}\sin (\lambda)\cos (\lambda)\Big] \und{e}_{2}'+\Big[2vp \sin  (\lambda)\Big]\und{e}_{3}' $$
...
נציב נתונים:
$$\begin{aligned}
\boldsymbol{\omega} & =\dfrac{(-5,0,0)(8,0,0)}{64}\cdot(0,-2,2)-\dfrac{(2,0,0)\cdot(8,0,0)}{64}(0,-3,-1) \\[1ex]
