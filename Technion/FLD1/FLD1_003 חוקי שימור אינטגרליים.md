...
# משוואת ברנולי
עבור נוזלים בלתי דחיסים, ניתן להניח שצפיפותם אחידה. מאחר וברנולי ביצע את ניסוייו על נוזלים בלתי דחיסים, הנוסחה שהוא פיתח עבורם תקיפה רק להם.
...
$$\boxed {
{p}_{1}+\dfrac{1}{2}\rho {{u}_{1}}^{2}+\rho g{z}_{1}={p}_{2}+\dfrac{1}{2}{p}_{2}{{u}_{2}}^{2}+\rho g{z}_{2}
 }$$
...
$$\int_{0}^{d} U\mathbf{x}\cdot(-\mathbf{x})  \, \mathrm{d}y+\int_{0}^{d} (Ay^{2}+By+c)\mathbf{x}\cdot\mathbf{x} \, \mathrm{d}y=0  $$
כאשר בחרנו את פרופיל המהירות בצורה הפרבולית:
$$\mathbf{u}_{\text{out}}=(Ay^{2}+By+c)\mathbf{x}$$
אנו יודעים שכאשר $y=0$, המהירות היא $0$, ולכן $c=0$:
$$\mathbf{u}_{\text{out}}=(Ay^{2}+By)\mathbf{x}$$
אנו גם יודעים כי
$$\begin{gathered}
...
\end{gathered}$$
ולכן:
$$\mathbf{u}_{\text{out}}=(Ay^{2}-2Ady)\mathbf{x}$$
נציב בחזרה בשימור מסה:
$$\begin{gathered}
...
ולכן פרופיל המהירות:
$$\boxed {
\mathbf{u}_{\text{out}}=\dfrac{3U}{2d^{2}}y(2d-y)\mathbf{x}
 }$$
...
$$\dfrac{ \partial  }{ \partial t } \int_{V}^{} \rho\mathbf{u} \, \mathrm{d}V+\int_{0}^{d} \rho U\mathbf{x}(U\mathbf{x}\cdot(-\mathbf{x})) \, \mathrm{d}y+\int_{0}^{d} \mathbf{u}_{\text{out}}\rho(\mathbf{u}_{\text{out}}\cdot\mathbf{x}) \, \mathrm{d}y=\sum \mathbf{f}   $$

הנפח קבוע, הצפיפות קבועה ואחידה, והמהירות קבועה, כך שנוכל לאפס את האיבר הראשון:
$$\begin{gathered}
\rho\int_{0}^{d} -U^{2}\mathbf{x} \, \mathrm{d}y+\rho\int_{0}^{d} \lvert u_{\text{out}} \rvert^{2}\mathbf{x} \, \mathrm{d}y=\sum \dfrac{\mathbf{f}}{\rho}   \\[1ex]
-\rho U^{2}d\mathbf{x}+\rho  \dfrac{6}{5}U^{2}d \mathbf{x}=\sum \dfrac{\mathbf{f}}{\rho}
\end{gathered}$$
...
$$\sum \mathbf{f}=-\int  p_{\text{in}}(-\mathbf{x})\,\mathrm{d}A-\int  p_{\text{out}}\mathbf{x}\,\mathrm{d}A+\int _{\text{wall}}\boldsymbol{\tau} \, \mathrm{d}A+\int _{\text{wing}}\boldsymbol{\tau} \, \mathrm{d}A+\int _{\text{symmetry line}}\boldsymbol{\tau} \, \mathrm{d}A$$
הכוחות שפה (הגזירה) לאורך קו הסימטריה הוא אפסי - משיקולי *סימטריה*. 
הכוחות שפה על הקיר העליון הם גם אפס, כי נתון שבקיר:
$$\boldsymbol{\tau}=\mu \dfrac{\mathrm{d}u}{\mathrm{d}y}\bigg|_{y=d}^{} \mathbf{x}=0$$
...
נציב בחזרה בשימור מסה:
$$-\rho U^{2}d\mathbf{x}+\rho\dfrac{6}{5}U^{2}d\mathbf{x}+p_{\text{in}}d\mathbf{x}-p_{\text{out}}d\mathbf{x}-\mathbf{f}_{\text{drag}}=0$$
...
נציב בחזרה בשימור מסה:
$$\begin{gathered}
\left( -\rho U^{2}d+\rho  \dfrac{6}{5}U^{2}d+\dfrac{5}{8}\rho U^{2}d \right)\mathbf{x}-\mathbf{f}_{\text{drag}}=0 \\[1ex]
\boxed {
\mathbf{f}_{\text{drag}}=0.825\rho U^{2}d\mathbf{x}
 }
\end{gathered}$$
...
לפי שימור מסה:
$$\begin{gathered}
\int \rho {V_{a}\mathbf{x}}(-\mathbf{x}) \, \mathrm{d}A+\int \rho V_{3}\mathbf{y}(\mathbf{y}) \, \mathrm{d}A+\int \rho V_{b}\hat{\mathbf{n}}(\hat{\mathbf{n}}) \, \mathrm{d}A=0    \\[1ex]
...
משקול כוחות:
$$\begin{aligned}
 & \sum \mathbf{F}\cdot \mathbf{x}=0: &  & F_{w,x}-F_{k}=0 \\
 &  &  & F_{w,x}-\mu N=0 \\[1ex]
 & \sum \mathbf{F}\cdot \mathbf{y}=0: &  & N-mg+F_{w,y}=0 \\
 &  &  & N=mg-F_{w,y}
\end{aligned}$$
