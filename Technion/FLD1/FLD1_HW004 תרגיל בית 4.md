...
![[FLD1_HW004/FLD1_HW004 תרגיל בית 4 2024-07-28 12.07.04.excalidraw.svg]]
...
נבצע [[FLD1_003 חוקי שימור אינטגרליים#חוק שימור המסה האינטגרלי|שימור מסה האינטגרלי]] על שני חתכים כלליים לאורך התעלה:
$$\dfrac{ \partial  }{ \partial t } \int _{V}\rho \, \mathrm{d}V+\int _{\partial V}\rho \mathbf{q}\cdot\hat{\mathbf{n}} \, \mathrm{d}A=0 $$
נפח הבקרה קבוע בזמן ו-$\rho$ אחיד, כך שהאינטגרל השמאלי מתבטל. בנוסף, בכניסה וביציאה המהירות נתונה ע"י $u$:
$$\begin{gathered}
\int _{\partial V}\rho \mathbf{q}\cdot\hat{\mathbf{n}} \, \mathrm{d}A=0 \\[1ex]
b\rho \int_{0}^{h} u_{\text{in}}(-\mathbf{x})\cdot\mathbf{x}+u_{\text{out}}\mathbf{x}\cdot\mathbf{x} \, \mathrm{d}y =0  \\[1ex]
\int_{0}^{h}u_{\text{in}} \, \mathrm{d}y=\int_{0}^{h} u_{\text{out}} \, \mathrm{d}y  
\end{gathered}$$
תוצאה זו נכונה לכל שתי נקודות לאורך התעלה. לפיכך, $u$ לא תלוי ב-$x$, מה שמוביל לטענה מספר $(5)$:
...
![[FLD1_HW004/FLD1_HW004 תרגיל בית 4 2024-07-28 12.40.03.excalidraw.svg]]
...
\qquad \qquad =\int_{0}^{h} \dfrac{1}{2\mu}\left( \dfrac{p_{a}-p^{*}+\rho g{H}_{2}}{L} \right)(y^{2}-hy)+\dfrac{a\omega}{h} \, \mathrm{d}y \\[2ex]
\int_{0}^{h} \dfrac{1}{2\mu}\left( \dfrac{p^{*}-p_{a}-\rho g{H}_{1}-(p_{a}-p^{*}+\rho g{H}_{2})}{L} \right) (y^{2}-hy)+\dfrac{a\omega}{h}y-a\omega\, \mathrm{d}y=0 \\[1ex]
 ...
 \dfrac{1}{\mu}\left( \dfrac{p^{*}-p_{a}-(\rho g/2)({H}_{1}+{H}_{2})}{L} \right)\left( \dfrac{1}{3}h^{3}-\dfrac{1}{2}h^{2} \right)+\dfrac{1}{2}a\omega h-a\omega h=0 \\[1ex]
...
### סעיף ד'
אנו נגיע למצב מתמיד כאשר הספיקה דרך אחד מהאזורים ההיה $0$ (ואז גם בהכרח הספיקה מהאזור השני תהיה $0$). נבחר ב-${u}_{2}$. אזי, נדרוש ש:
$$\begin{gathered}
...
## שאלה 2
![[FLD1_HW004/Pasted image 20240729091458.png|book|500]]

### סעיף א'
נבחר את מערכת הצירים:
![[FLD1_HW004/FLD1_HW004 תרגיל בית 4 2024-07-29 12.14.03.excalidraw.svg]]
...
אם נבצע [[FLD1_003 חוקי שימור אינטגרליים#חוק שימור המסה האינטגרלי|שימור מסה אינטגרלי]] על חתך כללי בתעלה, נקבל כי:
$$\int_{0}^{h}u_{\text{in}}  \, \mathrm{d}z=\int_{0}^{h} u_{\text{out}} \, \mathrm{d}z  $$
ולכן, נוכל לומר ש-$u$ קבוע לאורך $x$. כלומר, טענה מספר $(5)$:
...
הגדלים המנורמלים כולם בסדר גודל של $O(1)$, כך שהמקדמים שלהם גם באותו סדר גודל:
$$\dfrac{p^{*}}{L}=\rho\omega ^{2}L=\mu \dfrac{u^{*}}{h^{2}}$$
...
### סעיף ד'
![[FLD1_HW004/FLD1_HW004 תרגיל בית 4 2024-07-29 12.42.44.excalidraw.svg]]
...
## שאלה 3
![[FLD1_HW004/Pasted image 20240729125437.png|book]]
>סכימת הפלטות
...
$$\begin{gathered}
{u_{r}}^{*}=\dfrac{\pu{1m}}{\pu{2cm}}\cdot \pu{1mm.s^{-1}} \\[1ex]
\boxed {
...
\dfrac{{h}_{0}}{R} \dfrac{\rho {u_{r}}^{*}{h}_{0}}{\mu}\ll  1 \\[1ex]
\dfrac{{h}_{0}\rho {u_{r}}^{*}}{\mu} \ll 1
\end{gathered}$$
...
![[FLD1_HW004/FLD1_HW004 תרגיל בית 4 2024-07-29 15.20.12.excalidraw.svg]]
...
![[FLD1_HW004/Pasted image 20240729173043.png|book|250]]
>סכימת הבוכנה
...
![[FLD1_HW004/FLD1_HW004 תרגיל בית 4 2024-07-29 22.29.35.excalidraw.svg]]
>דג"ח רק בכיוון $x$ על הבוכנה

נתון כי התנועה קבועה, כך שמתקיים שימור תנע על הבוכנה ($m\mathbf{a}=0$). לכן, מהדג"ח, ומשימור תנע בכיוון $\mathbf{x}$:
$$\begin{aligned}
 & \sum \mathbf{F}\cdot \mathbf{x}=0: &  & mg+2b\int_{0}^{L} \tau \, \mathrm{d}x +b\int_{0}^{L} p_{0} \, \mathrm{d}y-b\int_{0}^{L} p_{L} \, \mathrm{d}y=0
\end{aligned}$$
...
![[FLD1_HW004/FLD1_HW004 תרגיל בית 4 2024-07-30 16.10.43.excalidraw.svg]]
...
נוכל כעת [[FLD1_004 משוואות נאוויה-סטוקס#משוואות נאוויה-סטוקס לנוזל בלתי דחיס|משוואות נאוויה-סטוקס]] את מאמץ הגזירה שנוצר כתוצאה ממהירות זו. לפי הגדרת הצמיגות:
...
\end{gather}$$
נציב את הפרש הלחצים בהגדרה שלנו ל-$K$:
$$\begin{gather}
...
![[FLD1_HW004/FLD1_HW004 תרגיל בית 4 2024-07-30 17.19.26.excalidraw.svg]]
...
\end{aligned}$$
השינוי בגובה $H$ הוא פשוט המהירות $U$:
$$\dfrac{ \partial A }{ \partial t } =-LU$$
לגבי המהירות על שפת נפח הבקרה - ישנן שתי יציאות, שתיהן במהירות $u$ שחישבנו עבור התעלה. נציב:
$$\begin{gathered}
-LU+2\int_{0}^{h} u\mathbf{x}\cdot(-\mathbf{x}) \, \mathrm{d}y=0  \\[1ex]
-LU-2\int_{0}^{h} \dfrac{1}{2}K(y^{2}-hy)+\dfrac{U}{h}y \, \mathrm{d}y =0 \\[1ex]
-LU-K\left( \dfrac{1}{3}h^{3}-\dfrac{1}{2}h^{2} \right)-U=0 \\[1ex]
-LU+\dfrac{1}{6}Kh^{3}-Uh=0 \\[1ex]
...
U(L+h)^{2}=\dfrac{gLh^{3}(\rho_{p}-\rho)}{6\mu}-\dfrac{h^{2}U}{3} \\[1ex]
U\left[ (L+h)^{2}+\dfrac{1}{3}h^{2} \right]=\dfrac{gLh^{3}(\rho_{p}-\rho)}{6\mu}
\end{gathered}$$
