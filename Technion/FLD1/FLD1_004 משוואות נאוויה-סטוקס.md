...
\begin{aligned}
 ...
 & \rho\left( \dfrac{ \partial u_{\theta} }{ \partial r } +u_{r} \dfrac{ \partial u_{\theta} }{ \partial r } +u_{z} \dfrac{ \partial u_{\theta} }{ \partial z } +\dfrac{1}{r}\left( u_{\theta}\dfrac{ \partial u_{\theta} }{ \partial \theta } +u_{r}u_{\theta} \right) \right)  \\[1ex]
 & =-\dfrac{1}{r}\dfrac{ \partial p }{ \partial \theta } +\rho g_{\theta}+\mu\left( \dfrac{ \partial  }{ \partial r } \left( \dfrac{1}{r}\dfrac{ \partial (ru_{\theta}) }{ \partial r }  \right)+\dfrac{1}{r^{2}}\dfrac{ \partial ^{2}u_{\theta} }{ \partial \theta ^{2} } +\dfrac{1}{r^{2}}\dfrac{ \partial ^{2}u_{\theta} }{ \partial r } +\dfrac{2}{r^{2}} \dfrac{ \partial u_{r} }{ \partial \theta } +\dfrac{ \partial ^{2}u_{\theta} }{ \partial z^{2} }  \right) \\[3ex]
...
# נורמליזציית משוואות נאוויה-סטוקס
...
\end{aligned} $$
כאשר $\text{Fr}$ הוא [מספר פראוד](https://en.wikipedia.org/wiki/Froude_number); $\text{Eu}$ הוא [מספר אויילר](https://en.wikipedia.org/wiki/Euler_number_(physics)); $Re$ הוא [מספר ריינולדס](https://en.wikipedia.org/wiki/Reynolds_number).

מבחינת משמעות אינטואיטיבית:
$$\begin{aligned}
\mathrm{Fr}^{-1/2}=\dfrac{\text{Gravity}}{\text{Inertia}} &  & \mathrm{Eu}=\dfrac{\text{Pressure}}{\text{Intertia}} &  & \mathrm{Re}^{-1}=\dfrac{\text{Viscosity}}{\text{Inertia}}
\end{aligned}$$
