...
$$L'=\dfrac{{V}_{0}+V_{L}}{A}$$
מעצם ההגדרות, $\Delta h=L'-L$. כלומר:
$$\begin{aligned}
\Delta h & =L'-L \\[1ex]
 & =\dfrac{{V}_{0}+V_{L}}{A}-L \\[1ex]
...
$$p_{o}=p_{a}-\rho_{o}gz$$
נשים לב ש-$r$ מקיים:
$$r=-z\cos 45^{\circ} \implies z=-\sqrt{ 2 }r $$
נסכם ש:
$$\begin{aligned}
 & p_{w}=p_{a}+\rho_{w}g\left( 1+\sqrt{ 2 }r\right) &  & p_{o}=p_{a}+\sqrt{ 2 }\rho_{o}gr
\end{aligned}$$
נציב בחזרה בביטוי ל-$M$:
$$\begin{aligned}
M & =b\int_{0}^{2} \left[ p_{a}+\rho_{w}g\left( 1+\sqrt{ 2 }r \right) \right]r-[p_{a}+\sqrt{ 2 }\rho_{o}gr]r \, \mathrm{d}r  \\[1ex]
 & =bg\int_{0}^{2} \rho_{w}(r+\sqrt{ 2 }r^{2})-\sqrt{ 2 }p_{o}r^{2} \, \mathrm{d}r  \\[1ex]
 & =bg\int_{0}^{2} \rho_{w}r+(\sqrt{ 2 }\rho_{w}-\sqrt{ 2 }\rho_{o})r^{2} \, \mathrm{d}r \\[1ex]
 & =bg\left[ \dfrac{1}{2}\rho_{w}r^{2}+\dfrac{1}{3}(\sqrt{ 2 }\rho_{w}-\sqrt{ 2 }\rho_{o})r^{3} \right]_{0}^{2} \\[1ex]
 & =bg\left[ 2\rho_{w}+\dfrac{8}{3}(\sqrt{ 2 }\rho_{w}-\sqrt{ 2 }\rho_{o}) \right]
\end{aligned}$$
נציב נתונים נקבל:
$$M=\pu {38357N.m }$$
...
$$\begin{aligned}
F & =\dfrac{M}{2} \\[1ex]
 & =\dfrac{bg}{2}\left[ 2\rho_{w}+\dfrac{8}{3}\left( \dfrac{1}{\sin\theta}\rho_{w}-\dfrac{1}{\sin\theta}\rho_{o} \right) \right]
\end{aligned}$$
