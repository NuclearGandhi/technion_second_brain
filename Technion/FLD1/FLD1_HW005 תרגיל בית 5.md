...
v(x,y) & =\dfrac{ \partial \phi }{ \partial y }  \\[1ex]
 & =\dfrac{\Gamma}{2\pi}\left[\dfrac{1/x}{1+[(y-a)/x]^{2}} -\dfrac{\Gamma}{2\pi}\left[\dfrac{1/x}{1+[(y+a)/x]^{2}} \right]\right]
\end{aligned}$$
ואכן, כאשר $y=0$:
$$\begin{aligned}
...
כאשר $y=0$:
$$\begin{aligned}
\mathbf{u}(x,0) & =u(x,0)\mathbf{x}+v(x,0)\mathbf{y} \\[1ex]
 & =\left( \dfrac{\Gamma}{2\pi x^{2}}\left[ \dfrac{a}{1+a^{2}/x^{2}}-\dfrac{-a}{1+a^{2}/x^{2}} \right]+U_{\infty } \right)\mathbf{x} \\[1ex]
 & =\left( \dfrac{a\Gamma}{2\pi x^{2}}\left[ \dfrac{x^{2}}{x^{2}+a^{2}}+\dfrac{x^{2}}{x^{2}+a^{2}} \right] \right)\mathbf{x} \\[1ex]
 & =\left( \dfrac{a\Gamma}{\pi(x^{2}+a^{2})}+U_{\infty } \right)\mathbf{x}
\end{aligned}$$
...
$$\boxed {
\mathbf{u}(x,0)=\left(U_{\infty }- \dfrac{a\Gamma}{\pi(x^{2}+a^{2})} \right)\mathbf{x}
 }$$
### סעיף ב'
...
רחוק מהקיר, כאשר $y\to \infty$, אפשר לראות משדה הזרימה שקיבלנו בסעיף קודם שמתקיים $\mathbf{u}_{\infty}=U_{\infty}\mathbf{x}$. נציב:
$$\begin{gathered}
...
עבור האינטגרל השמאלי אנו ידועים איך לפתור מ[[../CAL1/CAL1_007 אינטגרל לא מסוים#אינטגרלים מידיים|אינטגרלים מידיים]]. האינטגרל הימני דורש שיטות שלא למדנו:
$$\begin{aligned}
...
\end{aligned}$$
לסיכום:
$$\boxed{F=\rho\Gamma\left( U_{\infty }-\dfrac{\Gamma}{4\pi aU_{\infty }} \right) }$$
...
u_{\theta}(r,\theta) & =\dfrac{1}{r} \dfrac{ \partial \phi }{ \partial \theta } \\[1ex]
 ...
 & =-U_{\infty }\sin\theta-\dfrac{U_{\infty }a^{2}}{r^{2}}+\dfrac{\Gamma}{2\pi r}
\end{aligned}$$
...
F_{y} & =b\int_{0}^{2\pi} \dfrac{\rho}{2}(a\omega-2U_{\infty }\sin\theta)^{2}\sin\theta R  \, \mathrm{d}{\theta}  \\[1ex]
 & =\dfrac{b\rho}{2}\int_{0}^{2\pi} (a\omega ^{2}-4a\omega U_{\infty }\sin\theta+4U_{\infty }\sin ^{2}\theta)\sin\theta \, R\mathrm{d}\theta
\end{aligned}$$
...
$$\begin{aligned}
\mathbf{g} & =-\dot{u}(t)\mathbf{y}
\end{aligned}$$
לכן:
$$\rho \dfrac{ \partial \mathbf{u} }{ \partial t } +\dfrac{1}{2}\rho(\nabla(\mathbf{u}\cdot\mathbf{u}))+\nabla p+\rho \dot{u}(t)\mathbf{y}=0 $$
מעצם הגדרת פונקציית הפוטנציאל, מתקיים $\nabla \phi=\mathbf{u}$. נציב:
$$\rho\left( \dfrac{ \partial \nabla \phi }{ \partial t }  \right)+\dfrac{1}{2}\rho(\nabla(\nabla \phi)^{2})+\nabla p+\rho \dot{u}(t)\mathbf{y}=0$$
נכפיל את שני הצדדים ב-$\mathrm{d}\mathbf{r}$ ($\mathrm{d}\mathbf{r}=\mathrm{d}x\mathbf{x}+\mathrm{d}y\mathbf{y}+\mathrm{d}z\mathbf{z}$):
$$\left( \rho\left( \dfrac{ \partial \nabla \phi }{ \partial t }  \right)+\dfrac{1}{2}\rho(\nabla(\nabla \phi)^{2})+\nabla p+\rho \dot{u}(t)\mathbf{y} \right)\cdot \mathrm{d}\mathbf{r}=0$$
...
p=p_{\infty }+\rho\left[ u(t)^{2}\left( \dfrac{1}{2}-2\cos ^{2}\theta \right)+R\dot{u}(t)\sin\theta\right]
\end{gathered}$$
באופן יותר מדויק:
...
F_{y} & =b\int_{0}^{2\pi} \rho_{\infty }\sin\theta R\, \mathrm{d}\theta \\[1ex]
 ...
 & \qquad + b\rho \int_{\pi}^{2\pi} -\dfrac{3}{2}u(t)^{2}\sin\theta R \, \mathrm{d}R 
\end{aligned}$$
...
נציב בחזרה:
$$\begin{aligned}
\dfrac{F_{y}}{b\rho} & =u(t)^{2}R-\dfrac{4}{3}u(t)^{2}R+\pi R^{2}\dot{u}(t)+3u(t)^{2}R \\[1ex]
