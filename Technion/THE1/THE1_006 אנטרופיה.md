...
\mathrm{d}U+p\,\mathrm{d}V=\mathrm{d}H-V\,\mathrm{d}p
\end{gather}
$$
...
$$
\Delta S_{\text{sys} }+\Delta S_{\text{sur} }=\sigma_{\text{isol}}
$$

נשים לב שמשוואה זו לא דורשת ששינוי האנטרופיה יהיה חיובי עבור המערכת וחיובי עבור הסביבה, אלא רק שסכומם יהיה חיובי.

>[!tip] טיפים:
 >1. כאשר שואלים האם תהליך מסוים אפשרי, עלינו לבדוק שני דברים: חוק ראשון, וחוק שני. בדרך כלל, כאשר נבדוק את חוק ראשון, נקבל תנאי כלשהו על המערכת. נציב את תנאי זה בחוק שני (למשל, נדרוש שהנצילות לא יותר גדולה מנצילות קרנו), ונראה אם אנו מקבלים סתירה.
 >2. כאשר אנו רוצים לבדוק האם חוק שני מתקיים בעזרת עיקרון גדילת האנטרופיה, יהיה לנו מאוד נוח להסתכל מנקודת מבט של הסביבה. אנו מניחים שבסביבה אין שום ייצור אנטרופיה, ולכן אנו יכולים לחשב את השינוי באנטרופיה פשוט כסכום של המעברי חום, $\dfrac{Q}{T}$, בלי איבר מעצבן ${\sigma}_{\text{isol}}$ שעבור המערכת עצמה אנו לא בהכרח יודעים מהו.
...
3. נחשב שינוי אנטרופיה, כאשר נסמן את טמפרטורות הסביבה ב-${T}_{0}$.
	$$
	\Delta S_{\text{tot}}^{(\text{air})}=\dfrac{Q_{\text{sur} }}{{T}_{0}}+m_{{A}_{1}}(S_{{A}_{2}}-S_{{A}_{1}})+\Delta m(S_{{A}_{2}}-{S}_{0})
	$$
	לפי [[#שינוי אנטרופיה של גז אידיאלי]]:
	$$
	\begin{aligned}
