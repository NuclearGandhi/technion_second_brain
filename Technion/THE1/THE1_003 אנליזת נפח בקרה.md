...
![[THE1_003/Screenshot_20240206_085245_OneDrive.jpg|book|450]]
בזמן $t$, המסה היא הסכום:
$$m=m_{\text{cv}}(t)+m_{i}$$
כאשר $m_{\text{cv}}(t)$ היא המסה בתוך הנפח בקרה, ו-$m_{i}$ היא המסה באיזור הקטן המסומן $i$.

בטווח הזמן $\Delta t$ כל המסה באיזור $i$ עוברת לנפח בקרה, ואילו חלק מהמסה בנפח בקרה, שנסמן ב-$m_{e}$, יוצאת לאיזור $e$.
...
![[THE1_003/Screenshot_20240206_091100_OneDrive.jpg|book|400]]
...
>[!def] הגדרה: 
 ...
 >
 >נאמר כי הזרימה היא **חד מימדית**.

כאשר הזרימה חד מימדית, נקבל:
$$\boxed {
\dot{m}=\rho AV_{n}
 }$$
...
בעזרת הנוסחה מ[[#זרימה חד מימדית]] ($AV=\dot{m}v$):
$$\boxed {
\dot{W}=\dot{W}_{\text{cv}}+\dot{m}_{e}(p_{e}v_{e})-\dot{m}_{i}(p_{i}v_{i})
 }$$

## זרימה חד מימדית בחוק ראשון
נציב את הנוסחה האחרונה שפיתחנו ב[[#פיתוח הנוסחה לקצב שינוי באנרגיה|בנוסחה לקצב שינוי באנרגיה]] (כלומר, אנחנו מניחים שהזרימה היא חד מימדית):
...
מעבר לכך, בתהליך מתמיד מתקיים גם $\frac{\mathrm{d}E_{\text{cv}}}{\mathrm{d}t}=0$, ולכן לפי ה[[#זרימה חד מימדית בחוק ראשון|נוסחה לקצב שינוי אנרגיה]]:
...
העבודה היחידה שיכולה להתצבע בנפחי בקרה אילו הוא עבודת הזרימה, ולכן $W_{\text{cv}}=0$. בנוסף, מעברי חום עם הסביבה אמנם מתרחשים, אבל הם זניחים לעומת האנתלפיה והאנרגיה הקינטית. השינוי באנרגיה הפוטנציאלית מהכניסה ליציאה לרוב גם כן זניח. לכן משוואת האנרגיה עבורם:
$$\begin{gather}
...
## טורבינה
**טורבינה** היא מכשיר בו הספק נוצר כתוצאה מזרימה של גז או נוזל דרך מספר להבות המחוברות למוט מסתובב.
![[THE1_003/Pasted image 20240206183019.png|book|300]]

מבחינת אנרגיה, סך האנרגיה הקינטית של המסה שעוברת דרך גבולות המערכת לוב קטנה מספיק כדי שנוכל להזניח אותה. באותו אופן עבור האנרגיה הפוטנציאלית. לכן נקבל:
$$\begin{gather}
...
## מדחס ומשאבה
**מדחסים ומשאבות** הם מכשירים בהם עבודה מתבצעת על החומר הזורם דרכם כדי לשנות את מצבו - לרוב כדי להגדיל את הלחץ שלו או הגובה שלו. עבור גז, קוראים למכשיר **מדחס**, ואילו עבור נוזל אנו קוראים למכשיר **משאבה**.
...
![[THE1_003/Pasted image 20240206193035.png|book|500]]
...
בטבלת קיטור אין לנו ערכים עבור לחצים נמוכים יותר מ-$\pu{5MPa}$ לנוזל דחוס. עבור לחצים נמוכים אילו אנחנו נוכל לבצע קירוב, בו אנחנו מניחים שנוזל דחוס מאוד דומה לנוזל רווי ב-$(x=0)$:
$$\begin{aligned}
&v\approx v_{f} \\
&u\approx u_{f} \\
