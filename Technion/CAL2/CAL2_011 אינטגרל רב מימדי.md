...
\end{aligned} $$
	...
	$$\begin{align}
\bar{x_{I}}&=\frac{1}{M_{I}}\iint_{I} x \rho(x,y)\, \mathrm{d}x\mathrm{d}y=\frac{1}{m_{I}}\int_{\frac{\pi}{2}}^{3\pi/2} \int_{0}^{2} \,\underbrace{ \frac{1}{r} }_{ \rho }\cdot \underbrace{ r\cos \theta }_{ x } \cdot \underbrace{ r }_{ J }\, \mathrm{d}r  \, \mathrm{d}\theta  \\
\bar{x_{II}}&=\frac{1}{m_{II}}\iint_{II} x \rho(x,y)\, \mathrm{d}x\mathrm{d}y=\frac{1}{m_{II}}\int_{-\frac{\pi}{2}}^{\pi/2} \int_{2\cos \theta}^{2} \frac{1}{r} \, r\cos \theta \cdot r \, \mathrm{d}r  \, \mathrm{d}\theta 
\end{align}$$
...
לכן, נסיק כי:
>[!info] נוסחה: 
> $$\iiint_{D}f(x,y,z)\, \mathrm{d}x\mathrm{d}y\mathrm{d}z=\iiint f(\rho \cos \theta ,\rho \sin \theta \cos \varphi,\rho \sin \theta \cos \varphi)\,\rho ^{2}\sin \theta\,\mathrm{d}\rho \mathrm{d}\varphi \mathrm{d}\theta$$
...
\end{gather}$$
	ולכן:
	$$\begin{aligned}
\int_{0}^{2\pi} \int_{0}^{\pi/2} \int_{0}^{2a\cos \varphi} r\cdot r^{2}\sin \varphi \, \mathrm{d}r \, \mathrm{d}\varphi  \, \mathrm{d}\theta &=\int_{0}^{2\pi} \int_{0}^{\pi/2} \sin \frac{\varphi r^{4}}{4} \bigg|_{0}^{2a\cos \varphi}  \, \mathrm{d}\varphi \, \mathrm{d}\theta \\
&=\int_{0}^{2\pi} \int_{0}^{\pi/2} \sin \varphi 4a^{4}\cos ^{4}\varphi  \, \mathrm{d}\varphi  \, \mathrm{d}\theta \\
&=2\pi \cdot 4a^{4} \frac{-\cos ^{5}\varphi}{5}\bigg|_{0}^{\pi/2}   
\end{aligned} $$
