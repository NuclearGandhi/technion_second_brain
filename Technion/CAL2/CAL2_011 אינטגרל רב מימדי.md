...
עבור פונקציה כללית $f(x,y)$, נוכל להפוך אותה לפונקציה יותר נוחה ע"י הצבה מהצורה:
$$\begin{gather}
x=g(u,v) \\
y=h(u,v)
\end{gather}$$
...
>[!info] הגדרה:
> תהי טרנספורמציית החלפת המשתנים $T:\mathbb{R}^{2}\to \mathbb{R}^{2}$ (לא בהכרח לינארית!) מהצורה $T(x,y)=(g(x,y),h(x,y))=(u,v)$. אם מתקיימים התנאים הבאים:
...
### טרנספומציית המעבר לקואורדינטות פולאריות
...
>$$\iint_{B}f(x,y,z)\mathrm{d}V=\int_{e}^{f} \int_{c}^{d} \int_{a}^{b} f(x,y,z) \, \mathrm{d}x   \mathrm{d}y   \mathrm{d}z $$

אינטגרל זה שווה גם לכל אחד מחמשת האפשרויות האחרות של האינטגרל הנשנה המשולש. אז כל ששת האפשרויות הן:
...
> אז נבצע:
> $$\iint_{D}\left( \int_{x^{2}+3y^{2}}^{8-x^{2}-y^{2}} 1 \, \mathrm{d}z  \right)\mathrm{d}x\mathrm{d}y$$
