 ...
 **הוכחה:**
נניח תחילה שהתחום $D$ *פשוט* בשני הכיוונים, כאשר הפונקציות $g(x)$ ו-$h(x)$ חוסמות אותו. נקרא לשני תת-עקומות אילו ${C}_{1},{C}_{2}$.
...
>[!info] משפט: 
 ...
 >[!notes] הערות: 
 >
 >1. ספציפית עבור השדה $\vec{F}=-\frac{y}{2}\hat{\, \mathbf{i}}+\frac{x}{2}\hat{\, \mathbf{j}}$:
...
תהי $D$ תחום בעלת מספר סופי של "חורים" (כך של-$D$ יש מספר סופי של עקומות החוסמות אותו). נסמן  את העקומה החיצונית של $D$ ב-$\partial D$. נחלק את $D$ לשני תחומים ${D}_{1},{D}_{2}$, כל ש- $D={D}_{1} \cup {D}_{2}$, לשני תחומים אילו אין חורים:
