---
aliases: טרנספורמציה לינארית, אופרטור לינארי, מטריצת מעבר, מטריצה מייצגת, דמיון מטריצות, מטריצה דומה, שמורת דמיון
---
### טרנספורמציה לינארית
>[!info] הגדרה:
תהי $T:V\to U$ **טרנספורמציה** כש $V$ ו-$U$ 2 מרחבים וקטוריים, מעל אותו [[Technion/ALG1/ALG1_001 שדה#שדה|שדה]].
...
**סימון:** ט"ל שמוגדרת ע"י [[Technion/ALG1/ALG1_003 מטריצה#מטריצה|מטריצה]] $A$ מסומנת לעיתים ב-$T_{A}$.
...
\end{align}$$
>2. יהי $V$ [[Technion/ALG1/ALG1_005 מרחב וקטורי#מרחב וקטורי|מ"ו]] $n$ מימדי, מעל [[Technion/ALG1/ALG1_001 שדה#שדה|שדה]] $F$, ו-$e=\{e_{1},e_{2},\dots,e_{n} \}$ [[Technion/ALG1/ALG1_006 בסיס ומימד#בסיס ומימד|בסיס]] סדור שלו. אזי $T(v)=[v]_{e}$ היא ט"ל $T:V\to F^{n}$ כי $[v+w]_{e}=[v]_{e}+[w]_{e}$ ו-$[\alpha \cdot v]_{e}=\alpha[v]_{e}$.
...
1. הראה שאם $\forall v\in F^{n}: T_{A}(v)=T_{B}(v)$ אז $A=B$.
	**פתרון:**
	...
	$$Av=T_{A}(v)=T_{B}(v)=Bv$$
	ולכן לפי [[Technion/ALG1/ALG1_003 מטריצה#שוויון במכפלת כל עמודה במטריצה גורר שוויון מטריצות|משפט]] מתקיים $A=B$.
...
>1. הגרעין $\ker(T)$ הוא [[Technion/ALG1/ALG1_005 מרחב וקטורי#מרחב וקטורי|ת"מ]] של $V$.
...
>[!info] טענה:
תמונות של [[Technion/ALG1/ALG1_005 מרחב וקטורי#קבוצה פורשת|קבוצה פורשת]] של $V$, פורשות את התמונה. כלומר אם $\{ v_{1},v_{2},\dots,v_{n} \}$ פורשת את $V$ אז:
...
- הקבוצה $B$ [[Technion/ALG1/ALG1_005 מרחב וקטורי#תלות לינארית|בת"ל]] כי:
	$$\begin{gather}
...
>[!info] משפט:
יהיו $V$ ו-$U$ מרחבים וקטוריים מעל אותו [[Technion/ALG1/ALG1_001 שדה#שדה|שדה]] $F$ ונניח $\{ v_{1},\dots,v_{n} \}$ בסיס של $V$, ו-$\{ u_{1},\dots,u_{{n}} \}$ תת-קבוצה כלשהי של $U$.
...
>
בהשאלה, [[Technion/ALG1/ALG1_003 מטריצה#מטריצה ריבועית|מטריצה ריבועית]] לא הפיכה נקראת סינגולרית (כי אז ל-$Av=0$ יש פתרון $v\neq \bar{0}$).
...
1. נתונה $T:\mathbb{R}_{3}[x]\to\mathbb{R}_{3}[x]$:
	...
	- מצאו מטריצה מייצגת לפי הבסיס:
		...
		**פתרון:**
		...
		\end{aligned}$$
		כלומר לפתור [[Technion/ALG1/ALG1_004 מערכת משוואות לינארית#מערכת משוואות לינארית|ממ"ל]].
		סמכו עלי, יוצא:
...
**תזכורת:** [[Technion/ALG1/ALG1_007 מטריצה הפיכה#מטריצה הפיכה|מטריצה הפיכה]]
>[!info] מסקנה:
>אם $T$ הפיך, אז:
>$$[T^{-1}]_{e}=([T]_{e})^{-1}$$
...
**הוכחה:**
נוכיח לפי [[Technion/ALG1/ALG1_003 מטריצה#שוויון במכפלת כל עמודה במטריצה גורר שוויון מטריצות|מסקנה]]:
...
### שמורת העקבה
>[!info] טענה:
>אם $A$ ו-$B$ דומות, אז יש להן אותה [[Technion/ALG1/ALG1_003 מטריצה#עקבה|עקבה]]:
...
a
\end{pmatrix}$$
	מצאו את $[[#הקשר בין מטריצה מייצגת ומטריצת מעבר|T]].
	**פתרון:**
