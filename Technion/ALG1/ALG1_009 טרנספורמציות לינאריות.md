...
>	\end{align}
>	$$
>2. יהי $V$ [[ALG1_005 מרחבים וקטוריים#מרחב וקטורי|מ"ו]] $n$ ממדי, מעל [[ALG1_001 שדות#שדה|שדה]] $F$, ו-$e=\{e_{1},e_{2},\dots,e_{n} \}$ [[ALG1_006 בסיס וממד#בסיס וממד|בסיס]] סדור שלו. אזי $T(v)=[v]_{e}$ היא ט"ל $T:V\to F^{n}$ כי $[v+w]_{e}=[v]_{e}+[w]_{e}$ ו-$[\alpha \cdot v]_{e}=\alpha[v]_{e}$.
...
>[!example] דוגמאות:
>1. יהי $T:\mathbb{R}_{2}[x]\to \mathbb{R}_{2}[x]$ כך ש-$T$ היא טרנספורמצית הגזירה. מצא בסיס וממד ל-$\ker(T)$ ו-$\mathrm{Im}(T)$.
>	- עבור $\ker(T)$: מבוקשים כל ה-$p(x)\in \mathbb{R}_{2}[x]$ כך ש-$p'(x)=0$.
>	$$
>	\ker(T)=\{ c \ | \ c\in\mathbb{R} \}
>	$$
>	בסיס: $\{ 1 \}$.
>	ממד: $\mathrm{dim}(\ker(T)=1)$
>	- עבור $\mathrm{Im}(T)$: ע"י גזירה, נגיע לכל $\mathbb{R}_{1}[x]$.
>	בסיס: $\{ 1,x \}$
>	ממד: $\mathrm{dim} (\mathrm{Im}(T))=2$
...
>	T(x,y,z)=(x-y,x-z,z-y)
>	$$
>	מצא בסיס וממד של $\ker(T)$ ו-$\mathrm{Im}(T)$.
>	- נדרוש:
>	$$
>	\begin{cases}
>	x-y=0 \\
...
#### ממד תמונת ט"ל שמוגדרת ע"י מטריצה, שווה לדרגתה
...
ולכן לפי [[ALG1_006 בסיס וממד#ממד מרחב השורות של מטריצה שווה לדרגתה|משפט]]:
$$
...
>	\ker(T)=\ker(A)
>	$$
#### ממד הגרעין של טרנספורמציה לינארית שמוגדרת ע"י מטריצה שווה למספר דרגות החופש שלה
>[!info] מסקנה:
> ממשפט זה, ו[[#משפט הממדים השני|משפט הממדים השני]] וגם [[ALG1_006 בסיס וממד#ממד מרחב הפתרונות של ממ”ל הומוגנית|ממשפט]] נובע כי:
>$$
...
> $$
#### ט"ל היא על אמ"ם ממד התמונה שלה שווה לממד הטווח
>[!info] מסקנה:
> הט"ל $T:V\to U$ היא על אמ"ם:
> $$
...
#### תנאי החח"ע של ט"ל
>[!info] מסקנה:
>הט"ל $T:V\to U$ היא [[CAL1_001 פונקציה#פונקציה חח"ע ועל|חח"ע]] אמ"ם:
...
>[!info] טענה:
> יהי $T:V\to V$ אופרטור לינארי, אזי $T$ הפיך אמ"ם $T$ חח"ע.
> או - $T$ הפיך אמ"ם $T$ על.
...
>[!example] דוגמאות:
>1. נדגים את הוכחת 3 על מ"ו דו-ממדי.
...
>[!info] טענה:
> תהיינה $A,B\in F^{n\times n}$. אזי $A$ ו-$B$ דומות אמ"ם הן מייצגות אותו אופרטור $T$ לפי 2 בסיסים $e$ ו-$f$.
...
- נניח ש-$A,B\in F^{n\times n}$ ודומות. כלומר קיימת $Q$ הפיכה כך ש:
	...
	\end{pmatrix}
	$$
	נבחר מ"ו $V$ שממדו $n$ ונבחר לו בסיס $e=\{ e_{1}, \dots ,e_{n} \}$ וניצור אופרטור $T$ עליו נדרוש:
	$$
	A=[T]_{e}
	$$
	אזי:
