...
>[!info] הגדרה: 
 > מד"ר מהצורה:
 > $$x^{n}y^{(n)}+a_{n-1}x^{n-1}y^{(n-1)}+\dots +{a}_{1}xy'+{a}_{0}y=g(x)$$
 > נקראת **משוואת אויילר**.
...
\ln x=t
\end{gather}$$
  ואז נכול לעבור למשוואה עם מקדמים קבועים.
 הפתרונות של המשוואה ההומוגונית המתאימה הם $x^{{r}_{1}},x^{{r}_{2}},\dots,x^{r_{n}}$ כאשר $r_{i}$ הם שורשי הפ"א. בניית הפ"א האופייני תעשה באופן הבא:
 עבור ביטוי מהצורה $x^{n}$ (או הנגזרת ה-$n$-ית של $y$), נציב:
