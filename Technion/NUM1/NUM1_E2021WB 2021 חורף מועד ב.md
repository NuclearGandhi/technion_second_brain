...
\tilde{I} & =\dfrac{3}{4}hf\left( \dfrac{1}{3}h \right)+\dfrac{1}{4}hf(h)  \\[1ex]
 ...
 & =hf(0)+\dfrac{h^{2}}{2}f'(0)+\dfrac{h^{3}}{6}f''(0)+\dfrac{5h^{4}}{108}f'''(0)+\dfrac{h^{5}}{2592}f^{(4)}({\xi}_{2})+\dfrac{h^{4}}{96}f^{(4)}({\xi}_{3})
\end{aligned}$$
...
\end{aligned}$$
נוכל למצוא כי השגיאה של קירוב זה מאוד דומה לשגיאה של הקירוב הקודם:
$${E}_{2}=-\dfrac{1}{216}\left( \dfrac{h}{2} \right)^{4}f'''(0)-\dfrac{1}{405}\left( \dfrac{h}{2} \right)^{5}f^{(4)}(\xi)$$

>[!TODO] לשאול את מארק למה לא מכפילים ב-$2$ 
 >

ע"י [[NUM1_009 אינטגרציה נומרית#אינטגרציית רומברג|אקסטרפולציית ריצ'רדסון]] נוכל לקבל:
$$I_{R}=\dfrac{2^{4}{I}_{2}-{I}_{1}}{2^{4}-1}$$
לכן השגיאה של שיטה זו תהיה (מציבים פשוט את הביטוי השני בשגיאה במקום $I$, כי הביטוי הראשון באקסטרפולציית ריצ'רדסון מתקזז):
$$\begin{aligned}
E_{R} & =-\left( \dfrac{1}{405} \right)\cdot\dfrac{2^{4}\left( \dfrac{h}{2} \right)^{5}-h^{5}}{2^{4}-1}f^{(4)}(\xi)
\end{aligned}$$
ולכן:
$$\boxed{{E}_{R}=-\dfrac{1}{12150}h^{5}f^{(4)}(\xi)}$$
