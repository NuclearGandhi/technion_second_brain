...
### סעיף ג'
נציב את המערכת המקורית, משוואה $(\text{12.4})$, ל-`python`:
$$m\ddot{x}+{c}_{1}\dot{x}+{c}_{3}\dot{x}^{3}+kx\left[ 1-\dfrac{{\ell}_{0}}{\sqrt{ a^{2}+{{x}_{\text{eq}}}^{2} }} \right]=\dfrac{a}{{ a^{2}+{{x}_{\text{eq}}}^{2} }}{M}_{0}\cos(\omega t) \tag{12.4}$$
...
כעת נוכל להציג את $\dfrac{\lvert A \rvert}{{M}_{0}}$ כתלות ב-$\omega$ (כמובן, נציב את $(\text{12.3})$ עבור ${c}_{\text{eq}}$):
