...
>[!theorem] משפט: משוואות אויילר-לגראנז'
...
במקרה הפרטי (שהוא נכון עבור רוב הבעיות שניתקל בהם), אם מתקיים ש- $T=T(\dot{q}_{i})$, כלומר שניתן לבטא את $T$ רק ע"י $\dot{q}_{i}$, וגם $V=V(q_{i})$, נוכל לרשום את משוואות אויילר-לגראנז' באופן הבא:
$$
\boxed {
...
>[!example] דוגמה:
>פתחו את משוואות התנועה של המערכת הבאה בעזרת משוואות אויילר-לגראנז':
>![[{D24405D0-03A8-48A3-AF78-586703EBA6FB}.png|bookhue]]
> נשתמש בקואורדינטות המוכללות ${q}_{1}={\theta}_{1},\,{q}_{2}={\theta}_{2}$, כך שמשוואות אויילר-לגראנז' במקרה שלנו הן מהצורה:
> $$
...
> \end{aligned}$$
> נציב את כל הביטויים שמצאנו במשוואות אויילר-לגראנז' לקבלת משוואות התנועה:
> $$
> \boxed {
> \begin{gathered}
...
נגזור את $T$ ו-$V$ כפי שנדרש לפי משוואות אויילר-לגראנז'. עבור ${q}_{1}=x$:
$$
\begin{aligned}
