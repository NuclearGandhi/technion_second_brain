...
$$\begin{gathered}
\mathrm{Re}\{ -\Omega ^{2}Ae^{i\Omega \tau}+i\Omega {c}_{\text{eq}}Ae^{i\Omega \tau}+Ae^{i\Omega \tau} \}=\mathrm{Re}\{ \cos(\Omega \tau) \} \\[1ex]
-\Omega ^{2}A+i\Omega {c}_{\text{eq}} A+A=1
\end{gathered}$$
נקבל ש:
$$A=\dfrac{1}{[-\Omega ^{2}+1]+i\left[ \dfrac{3}{4}\delta\Omega ^{3} \lvert A \rvert^{2}\right]}$$
...
>דיאגרמת עוצמה לפתרון נומרי וקירוב לפתרון.
...
![[Pasted image 20241213154401.png|bookhue|450]]
