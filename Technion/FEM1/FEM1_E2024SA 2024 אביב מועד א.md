...
ניתן לראות כי:
$$\begin{aligned}
 &{\zeta}_{1}=0: &  &  {\phi}_{1}(-1)=\dfrac{3}{4}+1-\dfrac{1}{4}-\dfrac{1}{2}=1 \\[1ex]
 &   &  & \dfrac{\mathrm{d}{\phi}_{1}}{\mathrm{d}\zeta}(-1)=-\dfrac{3}{4}-2+\dfrac{3}{4}+2=0 \\[2ex]
 & {\zeta}_{2}=0: &  & {\phi}_{1}(0)=0 \\[2ex]
 & {\zeta}_{3}=1: &  & {\phi}_{1}(1)=-\dfrac{3}{4}+1+\dfrac{1}{4}-\dfrac{1}{2}=0 \\[1ex]
 &  &  & \dfrac{\mathrm{d}{\phi}_{1}}{\mathrm{d}\zeta}(1)=-\dfrac{3}{4}+2+\dfrac{3}{4}-2=0
\end{aligned}$$
### סעיף ג'
...
$$\begin{aligned}
\mathbf{B} & =\dfrac{4}{h^{2}}\hat{\mathbf{B}} \\[1ex]
 & =\begin{pmatrix}
2+\dfrac{3}{2}\zeta-12\zeta ^{2} \\
\dots  \\
\dots  \\
\dots 
\end{pmatrix}
...
$$\begin{aligned}
{K}^{e}_{11} & =\int {B}_{1}E{I}_{}{B}_{1} \, \mathrm{d}x  \\[1ex]
 & =\int_{-1}^{1} (J^{-1})^{2}\hat{B}_{1}EI{B}_{1} J \, \mathrm{d}\zeta  \\[1ex]
 & =\dfrac{4EI}{h^{2}}\int_{-1}^{1} \left( 2+\dfrac{3}{2}\zeta-12\zeta ^{2} \right)^{2} \, \mathrm{d}x 
\end{aligned}$$
בשתי נקודות גאוס ${w}_{i}=1$ ו- $\zeta=\pm 1/\sqrt{ 3 }$. לכן:
$$\begin{aligned}
{K}^{e}_{11} & =\dfrac{4EI}{h^{2}}\left( 2+\dfrac{3}{2}\cdot \dfrac{1}{\sqrt{ 3 }} -12\cdot \dfrac{1}{3}\right)^{2}+\left( 2-\dfrac{3}{2}\cdot \dfrac{1}{\sqrt{ 3 }}-12\cdot \dfrac{1}{3} \right) \\[1ex]
 & =\dfrac{4EI}{h^{2}}\left( \dfrac{19}{4}-2\sqrt{ 3 } +(4+\sqrt{ 3 })^{2}\right) \\[1ex]
 & =\boxed {
(95+24\sqrt{ 3 }) \dfrac{EI}{h^{2}}
 }
\end{aligned}$$
...
### סעיף ג'
יש לנו שטף אפסי בצמתים $1,2,3,4,5$. לכן אין תרומה של שטף ב- $F^{g}_{1},F^{g}_{2},F^{g}_{3},F^{g}_{4},F^{g}_{5}$.

אבל, בצמתים $7,8,9,10,11,12,13$ יש לנו את תנאי השפה הבא (לפי [[FEM1_007 משוואת פואסון הדו-ממדית#צורה חלשה של משוואת פואסון|צורה חזקה של משוואת פואסון]]):
$$k \dfrac{ \partial T }{ \partial \hat{\mathbf{n}} } ={q}''_{s}\cos\theta$$
כאשר $\theta$ היא זווית מ-$\mathbf{e}_{2}$ לנורמל לשפה (הרי נתון ש-$q''_{s}$ בכיוון $\mathbf{e}_{2}$).

מאחר ואין כאן איבר הסעה $h(\mathbf{x})$, השטף לא משפיע על $K^{g}$. התרומה של תנאי שפה זה לוקטור העומס היא $\int_{\Gamma_t} q''_s\cos\theta \phi_i \mathrm{d}S$, והוא כמובן מתווסף רק באלמנטים הרלוונטיים המצוינים לעיל.

### סעיף ד'
נשים לב שבצמתים $5,6,7,1,14,13$ יש תנאי שפה דיריכלה, כך שכבר נוכל לרשום:
$${a}_{5}={a}_{6}={a}_{7}={T}_{1},\qquad {a}_{1}={a}_{14}={a}_{13}={T}_{2}$$
...
$$ [\mathbf{K}]^e = \sum_{p=1}^{g} \sum_{r=1}^{g} w_q{w}_{r} k[\hat{\mathbf{B}}]^{T}[\mathbf{J}]^{-T}[\mathbf{J}^{-1}][\hat{\mathbf{B}}] |\det(\mathbf{J})|\tag{E3.2}$$
...
3(1-{\zeta}_{2})-(1-{\zeta}_{2})-2.5(1+{\zeta}_{2})+3.5(1+{\zeta}_{2}) & 1.5(1+{\zeta}_{2})-1.5(1+{\zeta}_{2}) \\
3(1-{\zeta}_{2})+(1+{\zeta}_{1})-2.5(1+{\zeta}_{1})+1.5(1-{\zeta}_{1}) & 1.5(1+{\zeta}_{1})+1.5(1-{\zeta}_{1})
\end{pmatrix} \\[1ex]
 &  \phantom{\,[\mathbf{J}]^{e=1}}=\begin{pmatrix}
-{\zeta}_{2}+3.5 & 0 \\
-6{\zeta}_{1}+3 & 3
\end{pmatrix} \\[3ex]
 & \det([\mathbf{J}])=3(-{\zeta}_{2}+3.5) \\[3ex]
 & ([\mathbf{J}]^{e=1})^{-1}=\dfrac{1}{3(-{\zeta}_{2}+3.5)}\begin{pmatrix}
3 & 0 \\
6{\zeta}_{1}-3 & -{\zeta}_{2}+3.5
\end{pmatrix}
\end{aligned}$$

נציב ב-$\text{(E3.1)}$ כאשר נשים לב שאכפת לנו רק מ-$K^{e=1}_{11}$:
$$\begin{aligned}
K^{e=1}_{11} & =2\cdot 1.5\cdot {B}_{11}\cdot {J}^{-1}_{11}\cdot J^{-1}_{11}\cdot {B}_{11}\cdot \lvert \det([\mathbf{J}]) \rvert  \bigg|_{\substack{{\zeta}_{1}=0\\ {\zeta}_{2}=0}}^{} \\[1ex]
 & =3\cdot \left( -\dfrac{1}{4} \right)\cdot\left( \dfrac{1}{10.5}\cdot 3 \right)\cdot \left( \dfrac{1}{10.5}\cdot 3 \right)\cdot\left( -\dfrac{1}{4} \right)\cdot 10.5 \\[1ex]
 & =3\cdot\left( \dfrac{1}{16} \right)\cdot \left( \dfrac{1}{10.5}\cdot 3 \right) \\[1ex]
 & = \boxed {
\dfrac{3}{56}
 }
\end{aligned}$$
