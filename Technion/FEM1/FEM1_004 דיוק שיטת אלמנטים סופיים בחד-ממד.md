...
אנו מתייחסים ל-$H^{h}_{u}(\Omega)$ ו-$H^{h}_{v}(\Omega)$ כמרחבי הקירוב (למשל, פונקציות לינאריות). הנקודה הקריטית היא ש-$H^{h}_{u}(\Omega), H^{h}_{v}(\Omega) \subset H^1(\Omega)$. קירוב "פנימי" זה מאפשר פיתוח של אומדני שגיאה פשוטים. אנו נבחר ב-$H^{h}_{u}(\Omega)$ ו-$H^{h}_{v}(\Omega)$ כזהים. יש לנו, עבור כל פונקציה קבילה $w \in H^1(\Omega)$, הגדרה של **סמי-נורמת האנרגיה (energy semi-norm)**:
...
>[!notes] הערה: 
 >עם איך ש-$\mathcal{B}$ מוגדר, ניתן לחשוב עליו בדיוק כמו אופרטור ב[[PDE1_005 בעיית שטורם ליוביל|מד"ח]].

נציין כי במקרה שמוגדרות הזזות לא אחידות על הגבול (ללא תנועה קשיחה), אזי $u-w=\text{const}$ אינו אפשרי אלא אם $u-w=0$ והסמי-נורמה במשוואה $\text{(4.2)}$ היא נורמה במובן המתמטי המדויק. תחת הנחות יחסית מתונות, אומדן שגיאה לשיטת האלמנטים הסופיים הוא:

$$||u - u^h||_{E(\Omega)} \leq \mathcal{C}(u,p)h^{\min(r-1,p):=\gamma} \tag{4.3}$$

כאשר $p$ הוא הסדר של הפולינום של שיטת האלמנטים הסופיים שבשימוש, $r$ הוא הרגולריות של הפתרון המדויק, ו-$\mathcal{C}$ הוא קבוע התלוי בפתרון המדויק ובקירוב הפולינומי. $\mathcal{C}$ אינו תלוי ב-$h$, שהוא הגודל המקסימלי של האלמנטים ברשת. לא ניכנס לפיתוח של ביטוי זה.
...
פתרון שיטת האלמנטים הסופיים הוא אופטימלי בנורמת האנרגיה. כדי להוכיח זאת אנו משתמשים ב-
...
כאשר $\forall v^h \in H^{v^h}(\Omega) \subset H^1(\Omega)$ וגם השגיאה מוגדרת ע"י $e^h := u - u^h$. תצפית חשובה היא שכל איבר של תת-המרחב ניתן לייצוג ע"י
...
%% ניתן להוכיח את תכונת המינימום של הפתרון המדויק באמצעות טכניקה חלופית. נבנה פונקציית פוטנציאל, עבור סטייה מהפתרון המדויק $u$, המסומנת $u + \lambda \nu$, כאשר $\lambda$ הוא סקלר ו-$\nu$ היא וריאציה קבילה כלשהי (פונקציית מבחן)

$$J(u+\lambda\nu) = \int_{\Omega}\frac{1}{2}\frac{d}{dx}(u+\lambda\nu)E\frac{d}{dx}(u+\lambda\nu)dx - \int_{\Omega}f(u+\lambda\nu)dx - t^*(u+\lambda\nu)|_{\Gamma_t}. \quad(4.16)$$

  

אם נגזור ביחס ל-$\lambda$,

  

$$\frac{\partial J(u+\lambda\nu)}{\partial \lambda} = \int_{\Omega}\frac{d\nu}{dx}E\frac{d}{dx}(u+\lambda\nu)dx - \int_{\Omega}f\nu dx - t^*\nu|_{\Gamma_t}, \quad(4.17)$$

  

ונציב $\lambda = 0$ (כי אנחנו יודעים שהפתרון המדויק הוא עבור $\lambda = 0$), נקבל

  

$$\frac{\partial J(u+\lambda\nu)}{\partial \lambda}|_{\lambda=0} = \int_{\Omega}\frac{d\nו}{dx}E\frac{du}{dx}dx - \int_{\Omega}f\nu dx - t^*\nu|_{\Gamma_t} = 0. \quad(4.18)$$

  

ברור שהממזער של הפוטנציאל הוא הפתרון למשוואות השדה, מכיוון שהוא מייצר את הצורה החלשה כתוצאה. זהו מינימום מכיוון ש-

  

$$\frac{\partial^2 J(u+\lambda\nu)}{\partial \lambda^2}|_{\lambda=0} = \int_{\Omega}\frac{d\nu}{dx}E\frac{d\nu}{dx}dx \geq 0. \quad(4.19)$$

  

חשוב לציין שהצורה החלשה, שפיתחנו קודם לכן, אינה דורשת פוטנציאל כזה ולכן היא גישה כללית יותר מעקרון מינימום. לכן, במקרים בהם קיימת אנרגיה פוטנציאלית, ניתן להתייחס לצורה החלשה כמזעור שלה. גישות מספריות המבוססות על רעיון זה מכונות בדרך כלל שיטות ריילי-ריץ (Rayleigh-Ritz). מושג זה מאפשר לבנות אומדני שגיאה פשוטים לזיקוק רשת גלובלית. %%
...
$$\begin{aligned}
||u - u^h||^2_{E(\Omega)} \leq C_1\sum^N_{e=1}h^2_e||r_1||^2_{L^2(\Omega_e)} + C_2\sum^{INT}_I h_{eI}||[|r_2|]||^2_{L^2(\partial\Omega_I)} + C_3\sum^{B-INT}_J h_{eJ}||r_3||^2_{L^2(\partial\Omega^B_J)},
\end{aligned} \quad(4.25)$$

כאשר
* $C_1$, $C_2$, ו-$C_3$ הם קבועים,
* $h_e$ הם הגדלים של האלמנטים,
* השארית הפנימית של האלמנט היא $r_1 = \frac{d}{dx}(E(x)\frac{du^h}{dx}) + f$,
* השארית ה"קופצת" של הממשק הפנימי היא $[|r_2|] = (E(x)\frac{du^h}{dx})_{x^+} - (E(x)\frac{du^h}{dx})_{x^-}$,
* שארית ממשק הגבול ("אי-שביעות רצון") היא $r_3 = (E(x)\frac{du^h}{dx}) - t^*$ על $\Gamma_t$, ו-
* אינדיקטורי שגיאה מקומיים מוגדרים על ידי

$$\zeta^2_e \stackrel{\text{def}}{=} C_1h^2_e||r_1||^2_{L^2(\Omega_e)} + C_2h_{eI}||[|r_2|]||^2_{L^2(\partial\Omega_I)} + C_3h_{eJ}||r_3||^2_{L^2(\partial\Omega^B_J)}. \quad(4.26)$$

הכמויות המקומיות $\zeta_e$ משמשות להחלטה האם אלמנט צריך להיות מזוקן (איור 4.3). אם $\zeta_e > TOL$, אז האלמנט מזוקן. אומדנים כאלה, המשמשים להנחיית טכניקות רשת אדפטיביות של אלמנטים סופיים מקומיים, פותחו לראשונה על ידי Babúska ו-Rheinboldt [9] עבור בעיות חד-ממדיות ועל ידי Babùska ו-Miller [10] ו-Kelly et al. [11] עבור בעיות דו-ממדיות. לסקירות ראו Ainsworth ו-Oden [1]. נושא זה יידון בהרחבה בסוף המונוגרפיה הזו.

![[FEM1_003 מימוש אלמנט סופי בחד-ממד 2025-04-21 16.34.19.excalidraw.svg|bookhue|500]]
>רשת מזוקקת מקומית ללכידת מאפייני פתרון עדינים יותר.
