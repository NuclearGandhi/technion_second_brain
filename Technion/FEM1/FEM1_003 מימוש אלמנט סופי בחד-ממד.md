...
# קירוב אלמנט סופי
נביט בצורה החלשה של הבעיה שראינו ב[[FEM1_002 אלסטוסטטיקה חד-ממדית#מרחב סובולב הילברטי|פרק קודם]] ([[FEM1_002 אלסטוסטטיקה חד-ממדית|משוואה]] $\text{(ZT2.13)}$):
...
\end{aligned}$$
את $\text{(ZT3.5)}$ אנו יכולים גם לרשום בצורה מטריציונית:
$$\boxed{\mathbf{K}\mathbf{a}=\mathbf{R} }\tag{ZT3.5b}$$
...
$$\phi(x)=\dfrac{x-{x}_{i-1}}{{h}_{i}}\qquad {x}_{i-1}\leq  x\leq  {x}_{i}\tag{ZT3.6}$$
כאשר ${h}_{i}={x}_{i}-{x}_{i-1}$ וגם
$$\phi(x)=1-\dfrac{x-{x}_{i}}{{h}_{i+1}}\qquad {x}_{i}\leq  x\leq  {x}_{i+1}\tag{ZT3.7}$$
אחרת, $\phi(x)=0$. הנגזרת של הפונקציה היא:
$$\dfrac{\mathrm{d}\phi}{\mathrm{d}x}=\dfrac{1}{{h}_{i}}\qquad {x}_{i-1}\leq  x\leq  {x}_{i}\tag{ZT3.8}$$
וגם
$$\dfrac{\mathrm{d}\phi}{\mathrm{d}x}=-\dfrac{1}{{h}_{i+1}}\qquad {x}_{i}\leq  x\leq  {x}_{i+1}\tag{ZT3.9}$$
...
- עבור סדר שלישי $(\zeta ^{3})$:
	$$\int_{-1}^{1} \zeta ^{3}  \, \mathrm{d}\zeta=0={w}_{1}{{{\zeta}_{1}}}^{3}+{w}_{2}{{{\zeta}_{2}}}^{3}\tag{ZT3.11} $$
- עבור סדר שני $(\zeta ^{2})$:
	$$\int_{-1}^{1} \zeta ^{2} \, \mathrm{d}\zeta =2/3={w}_{1}{{{\zeta}_{1}}}^{2}+{w}_{2}{{{\zeta}_{2}}}^{2}\tag{ZT3.12}$$
- עבור ביטוי לינארי $(\zeta)$:
	$$\int_{-1}^{1} \zeta \, \mathrm{d}\zeta =0={w}_{1}{\zeta}_{1}+{w}_{2}{\zeta}_{2}\tag{ZT3.13}$$
- עבור קבוע ($1$):
	$$\int_{-1}^{1} 1 \, \mathrm{d}\zeta =2={w}_{1}+{w}_{2}\tag{ZT3.14}$$
...
$$K^{e}_{ij}=\int_{{\Omega}_{e}}^{} \dfrac{\mathrm{d}{\phi}_{i}}{\mathrm{d}x}E \dfrac{\mathrm{d}{\phi}_{j}}{\mathrm{d}x} \, \mathrm{d}x \tag{ZT3.24}$$
וגם:
$$R^{e}_{i}=\int_{{\Omega}_{e}}^{} {\phi}_{i}f \, \mathrm{d}x +{\phi}_{i}t^{*}\bigg|_{{\Gamma}_{t,e}} \tag{ZT3.25}$$
...
$$x(\zeta)=\sum_{i=1}^{2}x_{i}\hat{\phi}_{i}(\zeta) \tag{ZT3.26} $$
...
$$J=\dfrac{\mathrm{d}x}{\mathrm{d}\zeta}\tag{ZT3.28}$$
ונשים לב גם ש:
$$\dfrac{1}{J}=\dfrac{\mathrm{d}\zeta}{\mathrm{d}x}\tag{ZT3.30}$$
...
\end{aligned}$$
נציב ב-$(\text{ZT3.24})$, ונקבל בעזרת [[#אינטגרציית גאוס]]:
$$\boxed {
\begin{aligned}
...
> \end{aligned} $$
> כמובן שאת כל האינטגרלים נחשב בעזרת אינטגרציית גאוס - כלומר, נשתמש ב- $\text{(ZT3.32)}$ ו-$\text{(ZT3.33)}$.
...
$$x(\zeta)={x}_{i}\hat{\phi}_{1}(\zeta)+{x}_{i+1}\hat{\phi}_{2}(\zeta)+{x}_{i+2}\hat{\phi}_{3}(\zeta) \tag{ZT3.48}$$
...
$$\sum_{j=1}^{N}\underbrace{ \int_{0}^{1/3} \dfrac{\mathrm{d}{\phi}_{i}}{\mathrm{d}x}E(x) \dfrac{\mathrm{d}{\phi}_{j}}{\mathrm{d}x} \, \mathrm{d}x   }_{ K^{e=1}_{ij} } \tag{ZT3.51}$$
מה שנותן:
$$\begin{align}
 ...
 & \qquad +\underbrace{ \int_{0}^{1/3} \dfrac{\mathrm{d}{\phi}_{1}}{\mathrm{d}x}E(x) \dfrac{\mathrm{d}{\phi}_{2}}{\mathrm{d}x}  \, \mathrm{d}x }_{ K^{e=1}_{14}=0 } +\underbrace{ \dots }_{ =0 } \tag{ZT3.52}  \\[1ex]
\end{align}$$

בצד ימין של $\text{(ZT3.51)}$, עבור $i=1,2,\dots$, אנו צריכים לחשב
$$\int_{0}^{1/3} {\phi}_{i}f(x) \, \mathrm{d}x =R^{e=1}_{i}\tag{ZT3.53}$$
ולכן
$$R^{e=1}_{1}=\int_{0}^{1/3} {\phi}_{1}f(x) \, \mathrm{d}x \tag{ZT3.54}$$
...
**פתרון**:
לפי [[FEM1_002 אלסטוסטטיקה חד-ממדית|משוואה]] $\text{(ZT2.14)}$, או לפי [[DVI1_008 רטט במערכות רציפות#משוואות התנועה של חוט תונד|משוואות תנועה של מוט במתיחה]]:
$$\boxed {
\begin{aligned}
...
נשים לב שבצד שמאל מופיע מטריצת הקשיחות ובצד ימין וקטור העומס, כמו ב[[#קירוב אלמנט סופי|משוואה]] $\text{(ZT3.5b)}$.
...
נרצה למצוא את $x(\zeta)$, כלומר, את $\zeta \to x$ עבור אלמנט מסוים. לפי [[#טרנספורמציה גלובלית/מקומית|משוואה]] $\text{(ZT3.26)}$:
$$\begin{aligned}
...
נסמן את הנגזרת לפי $\zeta$ עם ${\hat{B}}_{i}(\zeta):=\dfrac{\mathrm{d}{\hat{\phi}}_{i}(\zeta)}{\mathrm{d}\zeta}$, ונראה כי:
$${\hat{B}}_{1}(\zeta)=-\dfrac{1}{2},\qquad {\hat{B}}_{2}(\zeta)=\dfrac{1}{2}$$
הנגזרת לפי $x$:
$$\begin{aligned}
\dfrac{\mathrm{d}\phi^{e}_{i}(x)}{\mathrm{d}x} & =\dfrac{\mathrm{d}{\hat{\phi}}_{i}(\zeta)}{\mathrm{d}\zeta}\left( \dfrac{\mathrm{d}\zeta}{\mathrm{d}x} \right)^{e} \\[1ex]
 ...
 & ={\hat{B}}_{i}(\zeta) \dfrac{2}{{h}^{e}}
\end{aligned}$$
### סעיף ו'
מצאו את הקשיחות ה-*מקומית* ומטריצת המסה ה-*מקומית* של כל אלמנט.

**פתרון**:
לפי [[#מאפיינים דיפרנציאליים של פונקציות צורה|משוואות]] $\text{(ZT3.32)}$:
$$\begin{aligned}
{K}^{e}_{ij} & =\int_{{x}_{e}}^{{x}_{e+1}} \dfrac{\mathrm{d}\phi^{e}_{i}}{\mathrm{d}x} EA \dfrac{\mathrm{d}\phi^{e}_{j}}{\mathrm{d}x} \, \mathrm{d}x  \\[1ex]
 & =\int_{-1}^{1} \dfrac{1}{J^{e}}{\hat{B}}_{i}EA \dfrac{1}{J^{e}}{\hat{B}}_{j} \lvert J^{e} \rvert \, \mathrm{d}\zeta \\[1ex]
 & =\int_{-1}^{1} \dfrac{2}{h^{e}}{\hat{B}}_{i}EA \dfrac{2}{h^{e}}{\hat{B}}_{j} \dfrac{h^{e}}{2} \, \mathrm{d}\zeta  
\end{aligned}$$
ולכן:
$$\boxed{K^{e}_{ij}=\dfrac{2}{h^{e}}\int_{-1}^{1} {\hat{B}}_{i}EA{\hat{B}}_{j} \, \mathrm{d}\zeta  }$$
...
$$\begin{aligned}
K^{e}_{11} & =\dfrac{2}{h^{e}}\int_{-1}^{1} {\hat{B}}_{1}(x^{e}(\zeta)+2){\hat{B}}_{1} \, \mathrm{d}\zeta  \\[1ex]
 & =\dfrac{2}{1}\int_{-1}^{1} -\dfrac{1}{2}\left( \dfrac{1}{2}\zeta+ \dfrac{{x}_{e}+{x}_{e+1}}{2}+2 \right)\left( -\dfrac{1}{2} \right) \, \mathrm{d}\zeta \\[3ex]
K^{e}_{21} & =\dfrac{2}{h^{e}}\int_{-1}^{1} {\hat{B}}_{1}(x^{e}(\zeta)+2){\hat{B}}_{2} \, \mathrm{d}\zeta \\[1ex]
 & =\dfrac{2}{1}\int_{-1}^{1} -\dfrac{1}{2}\left( \dfrac{1}{2}\zeta+\dfrac{{x}_{e}+{x}_{e+1}}{2}+2 \right) \dfrac{1}{2} \, \mathrm{d}\zeta \\[3ex]
K^{e}_{22} & =\dfrac{2}{h^{e}}\int_{-1}^{1} {\hat{B}}_{2}(x^{e}(\zeta)+2){\hat{B}}_{2} \, \mathrm{d}\zeta \\[1ex]
...
לפי [[#טרנספורמציה גלובלית/מקומית|משוואה]] $\text{(ZT3.26)}$:
$$\begin{aligned}
