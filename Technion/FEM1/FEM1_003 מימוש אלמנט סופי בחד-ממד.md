...
>[!notes] הערה: 
 >בתרגילים אנו מפרקים את וקטור העומס $\mathbf{R}$ ל**מטריצת המסה** ו**וקטור הכוח**. אנו עושים זאת על קירוב $f$ בעזרת אותם הפונקציות בסיס:
 ...
 >ל-$\mathbf{f}$ אנו קוראים **וקטור הכוח** ולאינטגרל $\int_{\Omega}^{} {\phi}_{i}{\phi}_{j} \, \mathrm{d}x$ אנו קוראים **מטריצת המסה**.
...
>[!notes] הערה: 
 >1. התחום ${x}_{i-1}$ ל-${x}_{i}$ הוא מה שאנו קוראים לו **אלמנט סופי**.
 >2. אם אנו מחלקים את התחום ל-$N$ אלמנטים, אז יש לנו $N+1$ **צמתים** לאורך התחום.
...
כדי לבצע את החישובים באופן שיטתי, אנו רוצים צריכים להמיר את הקואורדינטות של כל אלמנט לתחום $[-1,1]$ לטובת אינטגרציית גאוס.  לפיכך, אנו צריכים לבצע את הטרנספורמציה הבאה, מהמערכת צירים המקומית למערכת הצירים המרחבית האמיתית,
...
>[!notes] הערה: 
 >אנו לא באמת מחשבים את הפונקציות ${\phi}_{i}$; אנחנו למעשה מתחילים מ- $\hat{\phi}_{i}$ ואז *ממפים* (טרנספורמציה) אותם לתחום של הבעיה האמיתית. לכן במטריצת הקשיחות או במטריצת המסה שנראה בהמשך, כל הביטויים חייבים להיות מוגדרים במונחים של קואורדינטות מקומיות.
 
כעת נגדיר מספר ביטויים בסיסיים שיעזרו לנו בחישוב הערכים המקומיים כמו $K^{e}_{ij}$ ו- $R^{e}_{i}$.  למשל, כאשר נבצע את הטרנספורמציה אסור לנו לשכוח שיש גם להכפיל את האינטגרנד ב[[CAL2_011 אינטגרל רב מימדי#מטריצת היעקוביאן|יעקוביאן]]. במקרה החד-ממדי:
