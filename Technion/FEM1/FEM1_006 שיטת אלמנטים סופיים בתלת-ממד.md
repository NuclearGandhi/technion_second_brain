...
\end{aligned} $$
מכיוון שהנגזרות $\frac{\partial\phi_k}{\partial x}$, $\frac{\partial\phi_k}{\partial y}$ (התלויות ב-$\mathbf{J}^{-1}$) והיעקוביאן $\det(\mathbf{J})$ הם פונקציות של $(\zeta_1,\zeta_2)$, האינטגרנד בדרך כלל אינו פולינום פשוט או קבוע. לכן, יש לבצע את האינטגרציה באופן נומרי. עבור אלמנטים מרובעים ביליניאריים, לרוב משתמשים ב-$2 \times 2$ נקודות גאוס כדי לקבל תוצאות מדויקות עבור אינטגרנדים עד סדר מסוים (בדרך כלל מספיק למקרים בהם $k$ קבוע או משתנה לינארית).
...
$$ K_{ij}^e = \int_{\Omega_e} k (\nabla\phi_i \cdot \nabla\phi_j) \mathrm{d}A $$
האינטגרל על האלמנט הפיזיקלי $\Omega_e$ מומר לאינטגרל על אלמנט המסטר $\Omega_M$:
$$ K_{ij}^e = \int_{-1}^{1}\int_{-1}^{1} k \left( \frac{\partial\phi_i}{\partial x}\frac{\partial\phi_j}{\partial x} + \frac{\partial\phi_i}{\partial y}\frac{\partial\phi_j}{\partial y} \right) |\det(\mathbf{J})| \mathrm{d}\zeta_1\mathrm{d}\zeta_2 $$

נוכל לרשום את חישובה באופן מטריצי:
$$ \begin{aligned}
\,[\mathbf{K}]^{e}  & = \int_{-1}^{1}\int_{-1}^{1} k[\mathbf{B}]^{T}[\mathbf{B}] |\det(\mathbf{J})| \mathrm{d}\zeta_1\mathrm{d}\zeta_2  \\[1ex]
 &  = \int_{-1}^{1}\int_{-1}^{1} k[\hat{\mathbf{B}}]^{T}[\mathbf{J}]^{-T}[\mathbf{J}^{-1}][\hat{\mathbf{B}}] |\det(\mathbf{J})| \mathrm{d}\zeta_1\mathrm{d}\zeta_2 
\end{aligned}$$
