...
נבחר אלמנט אב משולש במערכת קואורדינטות מקומית $(\xi, \eta)$. קונפיגורציה נפוצה היא משולש ישר זווית עם צמתים בנקודות:
*   צומת מקומי 1: $P'_1 = (0,0)$
*   צומת מקומי 2: $P'_2 = (1,0)$
*   צומת מקומי 3: $P'_3 = (0,1)$

פונקציות הצורה הלינאריות על אלמנט המסטר, המסומנות כ- $\hat{\phi}_k(\xi,\eta)$, הן:
$$ \begin{aligned}
...
המיפוי (טרנספורמציה) מקואורדינטות מקומיות $(\xi,\eta)$ לקואורדינטות גלובליות $(x,y)$ ניתן על ידי:
$$ \begin{aligned}
x(\xi,\eta) &= \sum_{k=1}^{3} \hat{\phi}_k(\xi,\eta) x_k = (1-\xi-\eta)x_1 + \xi x_2 + \eta x_3 \\
y(\xi,\eta) &= \sum_{k=1}^{3} \hat{\phi}_k(\xi,\eta) y_k = (1-\xi-\eta)y_1 + \xi y_2 + \eta y_3
\end{aligned} $$
...
הקשר בין אלמנט שטח דיפרנציאלי באלמנט המסטר, $\mathrm{d}\xi\mathrm{d}\eta$, לאלמנט שטח דיפרנציאלי באלמנט הפיזיקלי, $\mathrm{d}A = \mathrm{d}x\mathrm{d}y$, ניתן על ידי דטרמיננטת מטריצת היעקוביאן $\mathbf{J}$:
$$ \mathrm{d}A = |\det(\mathbf{J})| \mathrm{d}\xi\mathrm{d}\eta $$
...
$$ \begin{aligned}
 & \frac{\partial x}{\partial \xi} = x_2 - x_1 = x_{21}  & & \frac{\partial x}{\partial \eta} = x_3 - x_1 = x_{31} \\[1ex]
 & \frac{\partial y}{\partial \xi} = y_2 - y_1 = y_{21}  &  & \frac{\partial y}{\partial \eta} = y_3 - y_1 = y_{31}
\end{aligned} $$
...
$$\begin{aligned}
 \int_{\Omega_e} G(x,y) \mathrm{d}A  & = \int_{\Omega_M} G(x(\xi,\eta), y(\xi,\eta)) |\det(\mathbf{J})| \,\mathrm{d}\xi\mathrm{d}\eta \\[1ex]
 &  = 2A_e \int_{0}^{1} \int_{0}^{1-\eta} G(x(\xi,\eta), y(\xi,\eta)) \,\mathrm{d}\xi\mathrm{d}\eta 
\end{aligned}$$
...
*   נקודת אינטגרציה: $(\bar{\xi}, \bar{\eta}) = (1/3, 1/3)$
*   משקל: $w = A_M = 1/2$ (שטח אלמנט המסטר)

לפיכך, האינטגרל מקורב על ידי:
$$ \int_{\Omega_M} F(\xi,\eta) \,\mathrm{d}\xi\mathrm{d}\eta \approx F(\bar{\xi}, \bar{\eta}) \cdot w = F(1/3, 1/3) \cdot (1/2) $$
כאשר $(\bar{x}, \bar{y}) = (x(1/3,1/3), y(1/3,1/3))$ הוא מרכז הכובד של האלמנט הפיזיקלי.

### טרנספורמציה של נגזרות פונקציות הצורה
פונקציית הצורה הגלובלית $\phi_k(x,y)$ באלמנט הפיזיקלי זהה לפונקציית הצורה המקומית $\hat{\phi}_k(\xi,\eta)$ כאשר הקואורדינטות $(x,y)$ ו-$(\xi,\eta)$ קשורות על ידי המיפוי. כלומר, $\phi_k(x(\xi,\eta), y(\xi,\eta)) = \hat{\phi}_k(\xi,\eta)$.
הגרדיאנט של $\phi_k$ במונחים גלובליים קשור לגרדיאנט של $\hat{\phi}_k$ במונחים מקומיים על ידי:
$$ \begin{pmatrix} \frac{\partial \phi_k}{\partial x} \\ \frac{\partial \phi_k}{\partial y} \end{pmatrix} = (\mathbf{J}^T)^{-1} \begin{pmatrix} \frac{\partial \hat{\phi}_k}{\partial \xi} \\ \frac{\partial \hat{\phi}_k}{\partial \eta} \end{pmatrix} $$
כאשר:
$$ (\mathbf{J}^T)^{-1} = (\mathbf{J}^{-1})^T = \frac{1}{\det(\mathbf{J})} \begin{pmatrix} \frac{\partial y}{\partial \eta} & -\frac{\partial y}{\partial \xi} \\ -\frac{\partial x}{\partial \eta} & \frac{\partial x}{\partial \xi} \end{pmatrix} = \frac{1}{2A_e} \begin{pmatrix} y_{31} & -y_{21} \\ -x_{31} & x_{21} \end{pmatrix} $$
הנגזרות המקומיות של $\hat{\phi}_k$ הן:
$$\begin{aligned}
 & \nabla_{\xi\eta} \hat{\phi}_1 = \begin{pmatrix} -1 \\ -1 \end{pmatrix}  \\[1ex]
&  \nabla_{\xi\eta} \hat{\phi}_2 = \begin{pmatrix} 1 \\ 0 \end{pmatrix} \\[1ex]
 & \nabla_{\xi\eta} \hat{\phi}_3 = \begin{pmatrix} 0 \\ 1 \end{pmatrix}
\end{aligned}$$
...
$$ \begin{aligned}
\int_{\Omega_e} k(x,y) \mathrm{d}A  & = \int_{\Omega_M} k(x(\xi,\eta), y(\xi,\eta)) |\det(\mathbf{J})| \mathrm{d}\xi\mathrm{d}\eta  \\[1ex]
 & = 2A_e \int_{\Omega_M} k(x(\xi,\eta), y(\xi,\eta)) \mathrm{d}\xi\mathrm{d}\eta
\end{aligned} $$
נשתמש בכלל נקודת גאוס יחידה על אלמנט המסטר:
$$\begin{aligned}
 \int_{\Omega_M} k(x(\xi,\eta), y(\xi,\eta)) \mathrm{d}\xi\mathrm{d}\eta &  \approx k(x(1/3,1/3), y(1/3,1/3)) \cdot (1/2)  \\[1ex]
