...
> מבין 4 צלעות המלבן, רק הצלע האנגית הפנימית תורמת לאינטגרל (בחוץ השדה אפס, בצלעות אופקיות המסלול מאונך לכיוון השדה $\mathbf{B}\cdot \mathrm{d}\boldsymbol{\ell}=0$). לכן:
> $$\begin{aligned}
...
> $$\begin{gathered}
> \oint\mathbf{B}\cdot \mathrm{d}\boldsymbol{\ell}={\mu}_{0}I_{\text{in}} \\[1ex]
> Bb=\dfrac{b{\rho}_{0}\omega}{2}(a^{2}-r^{2}) \\[1ex]
> \boxed {
> B(r)=\begin{cases}
> 0 &  r>a \\[1ex]
> \dfrac{{\rho}_{0}\omega}{2}(a^{2}-r^{2}) & r<a
> \end{cases}
>  }
> \end{gathered}$$
> 
